          ...
          <div className="space-y-8">
            ...
            {featured && (
              <Card className="overflow-hidden py-0">
                <Link href={`/actualites/${featured.id}`} className="group grid gap-0 sm:grid-cols-2">
                  ...
                  <CardContent className="flex flex-col justify-center p-6 sm:p-8">
                    <div className="flex flex-wrap items-center gap-3 text-xs text-muted-foreground">
                      ...
                      <span className="inline-flex items-center gap-1.5">
                        <CalendarDays className="size-3.5" aria-hidden /> {formaterDate(featured.date)}
                      </span>
                      <span className="inline-flex items-center gap-1.5">
                        <Eye className="size-3.5" aria-hidden /> {featured.vues.toLocaleString("fr-FR")}
                      </span>
                    </div>
                    <h2 className="mt-4 text-balance font-heading text-xl font-semibold tracking-tight md:text-2xl">
                      {featured.titre}
                    </h2>
                    <p className="mt-3 text-sm leading-relaxed text-muted-foreground">{featured.extrait}</p>
            ...
            <div className="grid gap-6 sm:grid-cols-2">
              {reste.map((actuality) => (
                <Card key={actuality.id} className="overflow-hidden py-0">
                  <Link href={`/actualites/${actuality.id}`} className="group flex h-full flex-col">
                    ...
                    <CardContent className="flex flex-1 flex-col p-5">
                      <div className="flex flex-wrap items-center gap-2.5 text-xs text-muted-foreground">
                        ...
                        <span className="inline-flex items-center gap-1.5">
                          <CalendarDays className="size-3.5" aria-hidden /> {formaterDate(actuality.date)}
                        </span>
                      </div>
                ...
                {communiques.map((c, i) => (
                  <div key={c.id}>
                    ...
                    <div className="p-5">
                      ...
                      <p className="mt-2 text-sm leading-relaxed text-muted-foreground">{c.contenu}</p>
                      <p className="mt-3 text-xs text-muted-foreground">{formaterDate(c.date)}</p>
                    </div>
                  </div>
                ))}
              </CardContent>
