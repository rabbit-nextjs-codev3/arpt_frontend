...
function formaterChiffresCles(kpis: SectorOverview["kpis"]): ChiffreCle[] {
  ...
  return [
    {
      labelKey: "mobileSubscriptions",
      valeur: kpis.subscribersMillion,
      decimales: 1,
      suffixe: " M",
    },
    {
      labelKey: "internetSubscriptions",
      valeur: kpis.internetSubscribersMillion,
      decimales: 1,
      suffixe: " M",
    },
    {
      labelKey: "salariedJobs",
      valeur: kpis.salariedJobs,
      decimales: 0,
      suffixe: "",
    },
    {
      labelKey: "mobilePenetration",
      valeur: kpis.penetrationRate,
      decimales: 0,
      suffixe: " %",
    },
    {
      labelKey: "mobileMoneyPenetration",
      valeur: kpis.mobileMoneyPenetrationRate,
      decimales: 0,
      suffixe: " %",
    },
    {
      labelKey: "internetPenetration",
      valeur: kpis.internetPenetrationRate,
      decimales: 0,
      suffixe: " %",
    },
  ];
...
function extrait(html: string, maxLength = 140): string {
  const texte = html
    ...
    .replace(/\s+/g, " ")
    .trim();
  return texte.length > maxLength
    ? `${texte.slice(0, maxLength).trimEnd()}…`
    : texte;
}
...
const RACCOURCIS_DEFAUT: Raccourci[] = [
  ...
  },
  {
    ...
    label: "Indicateurs des marchés",
    icone: "statistiques",
    texte: "Observatoires officiels du secteur",
  },
];
  ...
  return (
    <div className="flex items-center justify-between gap-3">
      <div className="flex min-w-0 items-center gap-2.5">
        ...
        <h3 className="truncate font-heading text-base font-semibold whitespace-nowrap">
          {titre}
        </h3>
      </div>
      {lienVoirTout && (
    ...
    >
      <div className="relative min-h-32 bg-institution p-6 text-primary-foreground sm:p-8">
        ...
        <div className="relative pr-10">
          ...
          <h2 className="mt-3 font-heading text-xl font-semibold leading-tight sm:text-2xl">
            {actualite.title}
          </h2>
          <p className="mt-3 inline-flex items-center gap-1.5 text-xs text-primary-foreground/80">
            <CalendarDays className="size-3.5" aria-hidden />{" "}
            {formaterDate(actualite.createdAt)}
          </p>
        </div>
        <button
          type="button"
          ...
          className="absolute right-4 top-4 grid size-9 place-items-center rounded-full bg-primary-foreground/15 text-primary-foreground transition-colors hover:bg-primary-foreground/25"
        >
      ...
      <div className="p-6 sm:p-8">
        <p className="text-sm leading-7 text-muted-foreground">
          {extrait(actualite.content, 850)}
        </p>
        <div className="mt-6 flex justify-end border-t border-border pt-5">
          <Button asChild>
            <Link
              href={`/actualites/${actualite.uid}`}
              onClick={() => dialogRef.current?.close()}
            >
...
export default function Accueil() {
  ...
  const { data: raccourcis } = useContentBlock<Raccourci[]>(
    "home.quickLinks",
    RACCOURCIS_DEFAUT,
  );
  ...
  const { data: communiques } = useApiList<Communique>(
    "/communiques?lang=fr&pageSize=4",
  );
  const { data: textesLesPlusConsultes } = useApiList<Reglementation>(
    "/regulations?lang=fr&pageSize=1&ordering=-views",
  );
  const { data: appelsOffres } = useApiList<TendersCall>(
    "/tenders?lang=fr&pageSize=50",
  );
  const { data: consultations } = useApiList<PublicConsultation>(
    "/public-consultations?lang=fr&pageSize=3",
  );
  const { data: overview } = useApiOne<SectorOverview>(
    "/statistics/overview?lang=fr",
  );

  const offresOuvertes = appelsOffres.filter((a) => a.status === "OUVERT");
  const texteLePlusConsulte = textesLesPlusConsultes[0] ?? null;
  ...
  return (
    <>
      ...
      <section className="relative overflow-hidden bg-institution text-primary-foreground">
        ...
        />
        <div
          className="absolute inset-x-0 bottom-0 h-16 bg-gradient-to-b from-transparent to-institution"
          aria-hidden
        />
        <div className="relative container-content py-12 md:py-16">
          ...
          <p className="mt-4 max-w-2xl text-balance text-base leading-relaxed opacity-90 md:text-lg">
            {hero.description}
          </p>
            ...
            {raccourcis.map((r) => {
              ...
              return (
                <Link
                  key={r.to}
                  href={r.to}
                  className="group flex items-start gap-4 rounded-lg border border-border bg-surface p-5 transition-colors hover:border-primary/40 hover:bg-zinc-100 hover:shadow-soft"
                >
      ...
      <section className="section-y bg-surface">
        <div className="container-content grid gap-8 lg:grid-cols-3">
          ...
          <div className="space-y-10 lg:col-span-2">
            ...
            <div>
              <SectionHeader
                icon={Megaphone}
                titre={t("latestPublications")}
                lienVoirTout="/actualites"
              />
              <div className="mt-5 grid gap-4 md:grid-cols-2">
                {actualites[0] && (
                  <article className="group relative min-h-72 overflow-hidden rounded-2xl bg-institution text-primary-foreground shadow-card md:row-span-2">
                    ...
                    <div
                      className="absolute inset-0 bg-gradient-to-t from-primary-deep via-primary-deep/75 to-primary-deep/15"
                      aria-hidden
                    />
                    <div className="relative flex min-h-72 flex-col p-5 sm:p-6">
                      <div className="flex items-center justify-between gap-3">
                        {actualites[0].category && (
                          <Badge className="bg-primary-foreground/15 text-primary-foreground backdrop-blur-sm">
                            {actualites[0].category}
                          </Badge>
                        )}
                        <span className="inline-flex items-center gap-1.5 text-xs text-primary-foreground/75">
                          <CalendarDays className="size-3.5" aria-hidden />
                          {formaterDate(actualites[0].createdAt)}
                        </span>
                      </div>
                      <div className="mt-auto">
                        <h3 className="max-w-lg text-balance font-heading text-xl font-semibold leading-tight sm:text-2xl">
                          {actualites[0].title}
                        </h3>
                        <p className="mt-3 line-clamp-2 max-w-lg text-sm leading-6 text-primary-foreground/80">
                          {extrait(actualites[0].content, 180)}
                        </p>
                        <div className="mt-5 flex flex-wrap gap-2">
                          <Button asChild size="sm" variant="secondary">
                            <Link href={`/actualites/${actualites[0].uid}`}>
                              Lire l&apos;article{" "}
                ...
                {actualites.slice(1).map((a) => (
                  <article
                    key={a.uid}
                    className="group flex min-h-34 gap-4 rounded-xl border border-border bg-card p-3.5 shadow-soft transition-all hover:-translate-y-0.5 hover:border-primary/25 hover:shadow-card"
                  >
                    ...
                    <div className="flex min-w-0 flex-1 flex-col">
                      ...
                      <h3 className="mt-2 line-clamp-2 font-heading text-sm font-semibold leading-snug transition-colors group-hover:text-primary">
                        {a.title}
                      </h3>
                      <div className="mt-auto flex items-center justify-end pt-3">
                        <Link
                          href={`/actualites/${a.uid}`}
                          aria-label={`Lire ${a.title}`}
                          className="grid size-7 place-items-center rounded-full bg-teal-600 text-white shadow transition-colors hover:bg-teal-700"
                        >
                          <ArrowRight className="size-3.5" aria-hidden />
                        </Link>
                      </div>
                ...
                <CardContent className="p-0">
                  {communiques.map((c, i) => (
                    <div key={c.uid}>
                      ...
                      <Link
                        href="/actualites"
                        className="flex items-start gap-2.5 p-3.5 hover:translate-x-0.5 transition-transform hover:bg-zinc-100"
                      >
                        <FileText
       ...
       <div className="container-content mt-8 grid gap-8 lg:grid-cols-2">
  {/* Appels d'offres */}
  <div>
    <SectionHeader
      icon={Gavel}
      titre={t("ongoingTenders")}
      lienVoirTout="/appels-offres"
    />
    <Card className="mt-5">
      <CardContent className="p-0">
          ...
          <div key={a.uid}>
            ...
            <div className="flex items-start justify-between gap-4 p-4">
              <div className="min-w-0">
                ...
                <Link
                  href={`/appels-offres?uid=${encodeURIComponent(a.uid)}`}
  ...
  <div>
    <SectionHeader
      icon={MessageSquare}
      titre={t("mostViewedTexts")}
      lienVoirTout="/reglementation"
    />
    <Card className="mt-5">
      <CardContent className="p-5">
        <ul className="space-y-4">
          {texteLePlusConsulte && (
            <li>
              <Link
                href={`/reglementation/${texteLePlusConsulte.uid}`}
              ...
              >
                ...
                <span>
                  <span className="line-clamp-2 font-medium group-hover:underline">
                    {texteLePlusConsulte.name}
                  </span>
                  <span className="mt-1 inline-flex items-center gap-1 text-xs text-muted-foreground">
                    <Eye className="size-3" aria-hidden />
                    {texteLePlusConsulte.views.toLocaleString("fr-FR")}{" "}
                    vues
                  </span>
                </span>
              </Link>
      ...
      {actualiteApercu && (
        <ApercuActualite
          actualite={actualiteApercu}
          onClose={() => setActualiteApercu(null)}
        />
      )}
    </>
  );
