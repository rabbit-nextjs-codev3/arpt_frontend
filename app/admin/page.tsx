...
const menus = menuGroups
  .flatMap((g) => g.items)
  .map((m) => ({ ...m, href: `/admin?section=${m.id}` }));
...
function NotificationsBell({
  ...
  async function ouvrir() {
    ...
    setLoading(true);
    try {
      const res = await apiFetch<PaginatedResult<NotificationItem>>(
        "/notifications?pageSize=15",
      );
      ...
      {open && (
        <div className="absolute top-full right-0 z-20 mt-2 w-80 rounded-xl border border-border bg-card shadow-lg">
          ...
          <div className="max-h-96 overflow-y-auto">
            {loading && (
              <Loader className="flex justify-center p-4" label="Chargement" />
            )}
...
export default function Admin() {
  ...
  const { data: overview } = useApiOne<AdminOverview>(
    staff ? "/dashboard/admin-overview" : null,
  );
  const { data: overviewStats } = useApiOne<SectorOverview>(
    staff ? "/statistics/overview?lang=fr" : null,
  );
  const { data: reclamationsRecentes } = useApiList<ClaimAdmin>(
    staff ? "/claims?lang=fr&pageSize=3" : null,
  );
  const { data: newsReelles } = useApiList<{ id: number }>(
    staff ? "/news?pageSize=100" : null,
  );
  const { data: communiquesReels } = useApiList<{ id: number }>(
    staff ? "/communiques?pageSize=100" : null,
  );
  const { data: tendersReels } = useApiList<{ status: string }>(
    staff ? "/tenders?pageSize=100" : null,
  );
  const { data: careersReels } = useApiList<{ isNew: boolean }>(
    staff ? "/careers?pageSize=100" : null,
  );
  const { data: reglementationsReelles } = useApiList<{ id: number }>(
    staff ? "/regulations?pageSize=100" : null,
  );
  ...
  const kpisReels = overview
    ? [
        {
          libelle: t("dashboard.kpi.openClaims"),
          valeur: overview.reclamations?.ouvertes ?? "—",
        },
        {
          libelle: t("dashboard.kpi.candidatures"),
          valeur: overview.candidatures?.total ?? "—",
        },
        {
          libelle: t("dashboard.kpi.unreadMessages"),
          valeur: overview.messagesContact?.nonLus ?? "—",
        },
      ]
    : [];
  ...
  const indicateursSectoriels = overviewStats?.kpis
    ? [
        {
          libelle: t("dashboard.sectorKpi.mobileSubscribers"),
          valeur:
            overviewStats.kpis.subscribersMillion != null
              ? `${overviewStats.kpis.subscribersMillion.toLocaleString("fr-FR")} M`
              : "—",
          icon: Smartphone,
        },
        {
          libelle: t("dashboard.sectorKpi.penetrationRate"),
          valeur:
            overviewStats.kpis.penetrationRate != null
              ? `${overviewStats.kpis.penetrationRate} %`
              : "—",
          icon: TrendingUp,
        },
        {
          libelle: t("dashboard.sectorKpi.activeOperators"),
          valeur:
            overviewStats.kpis.activeOperators != null
              ? `${overviewStats.kpis.activeOperators}`
              : "—",
          icon: Users,
        },
        {
          libelle: t("dashboard.sectorKpi.active4GSites"),
          valeur:
            overviewStats.kpis.active4GSites != null
              ? overviewStats.kpis.active4GSites.toLocaleString("fr-FR")
              : "—",
          icon: TowerControl,
        },
      ]
  ...
  const variationCA =
    caReel.length >= 2
      ? ((caReel[caReel.length - 1].ca - caReel[caReel.length - 2].ca) /
          caReel[caReel.length - 2].ca) *
        100
      : null;
    ...
    ? [
        {
          ...
          detail: t("dashboard.todo.qualifyClaimsDetail", {
            count: overview.reclamations?.ouvertes ?? 0,
          }),
          icon: AlertCircle,
          tone: "text-warning",
        },
        {
          ...
          detail: t("dashboard.todo.unreadContactMessagesDetail", {
            count: overview.messagesContact?.nonLus ?? 0,
          }),
          icon: Newspaper,
          tone: "text-chart-2",
        },
        ...
        >
              ...
              <div className="relative">
                ...
                >
                  ...
                  <span className="hidden text-left sm:block whitespace-nowrap">
                    <span className="block text-xs leading-tight font-semibold">
                      {user.fullname}
                    </span>
                    <span className="block text-[11px] leading-tight text-muted-foreground">
                      {user.isSuperuser
                        ? t("header.superAdmin")
                        : (user.role?.name ?? t("header.administrator"))}
                    </span>
                  </span>
                  <ChevronDown
          ...
          <div className="min-w-0 bg-surface-fade p-6 lg:p-10">
                ...
                {indicateursSectoriels.length > 0 && (
                  <div>
                    <p className="mb-3 text-xs font-semibold tracking-[0.14em] text-muted-foreground uppercase">
                      {t("dashboard.sectorObservatory", {
                        year: overviewStats?.year ?? "",
                      })}
                    </p>
                    <dl className="grid gap-4 sm:grid-cols-2 md:grid-cols-4">
                      {indicateursSectoriels.map((k) => (
                        ...
                        >
                          ...
                          <dd className="mt-4 font-heading text-2xl font-bold text-foreground">
                            {k.valeur}
                          </dd>
                        </div>
                      ))}
                ...
                <div className="grid gap-6 grid-cols-2">
                  ...
                  <div className="min-w-0 rounded-xl border border-border bg-card shadow-soft">
                    <div className="flex items-center justify-between gap-4 border-b border-border px-5 py-4 lg:px-6">
                      <div>
                        <h2 className="font-heading text-lg font-semibold">
                          {t("dashboard.recentClaims.title")}
                        </h2>
                        <p className="mt-1 text-xs text-muted-foreground">
                          {t("dashboard.recentClaims.subtitle")}
                        </p>
                      </div>
                      <button
                        onClick={() => setSection("reclamations")}
                      ...
                      >
                        {t("dashboard.recentClaims.viewAll")}
                      </button>
                    </div>
                    <TableauAdmin
                      compact
                      colonnes={
                        t.raw("dashboard.recentClaims.columns") as string[]
                      }
                    ...
                    {reclamationsRecentes.length === 0 && (
                      <p className="px-5 py-6 text-center text-xs text-muted-foreground">
                        {t("dashboard.recentClaims.empty")}
                      </p>
                    )}
                  </div>
                </div>
            ...
            {section === "candidatures" &&
              (hasPermission("view_candidatures") ||
                hasPermission("view_submissions")) && (
                <ApplicationsAdmin
                  canViewCareers={hasPermission("view_candidatures")}
                  canViewTenders={hasPermission("view_submissions")}
                  canManageCareers={hasPermission("manage_carrieres")}
                  canManageTenders={hasPermission("manage_appels_offres")}
                />
              )}
...
const EQUIPMENT_STATUTS: EquipementAdmin["status"][] = [
  "HOMOLOGUE",
  "EN_COURS",
  "INTERDIT",
];
...
function EquipementsAdmin() {
  ...
  const {
    ...
    error,
    refetch,
  } = useApiList<EquipementAdmin>("/equipment?lang=fr&pageSize=100");
  const { data: categories } = useApiOne<EquipmentCategoryOption[]>(
    "/equipment-categories?lang=fr",
  );
  ...
  const resultat = equipements.filter((e) => {
    const correspondRecherche = `${e.code} ${e.name} ${e.brand} ${e.model}`
      .toLocaleLowerCase("fr")
      .includes(recherche.trim().toLocaleLowerCase("fr"));
    const correspondStatut =
      filtreStatut === "all" || e.status === filtreStatut;
    const correspondCategorie =
      filtreCategorie === "all" || e.category.slug === filtreCategorie;
  ...
  const resultatPage = resultat.slice(
    (pageCourante - 1) * PAGE_SIZE_ADMIN,
    pageCourante * PAGE_SIZE_ADMIN,
  );
  const homologues = equipements.filter((e) => e.status === "HOMOLOGUE").length;
  const enCoursCount = equipements.filter(
    (e) => e.status === "EN_COURS",
  ).length;
  ...
  async function soumettre(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (form.get("homologationNumber"))
      body.append("homologationNumber", String(form.get("homologationNumber")));
    if (form.get("validUntil"))
      body.append("validUntil", String(form.get("validUntil")));
      ...
      {showForm && (
        ...
        >
          <p className="text-sm font-semibold sm:col-span-2">
            {editing
              ? t("editingTitle", { name: editing.name })
              : t("newEquipmentTitle")}
          </p>
          ...
          <div className="grid gap-2">
            ...
            <Select
              value={status}
              onValueChange={(v) => setStatus(v as EquipementAdmin["status"])}
            >
          ...
          <div className="grid gap-2">
            ...
            <Input
              ...
              placeholder={t("fields.homologationOptional")}
              defaultValue={editing?.homologationNumber ?? ""}
            />
          </div>
          <div className="grid gap-2">
            ...
            <Input
              ...
              name="validUntil"
              type="date"
              defaultValue={editing?.validUntil?.slice(0, 10) ?? ""}
            />
          </div>
          <div className="grid gap-2 sm:col-span-2">
            <Label htmlFor="eq-certificate">
              {t("fields.certificate", {
                hint: editing
                  ? t("fields.fileHintKeep")
                  : t("fields.fileHintOptional"),
              })}
            </Label>
            <Input
              id="eq-certificate"
        ...
        <div className="overflow-hidden rounded-2xl border border-border bg-card shadow-card">
          <div className="grid gap-3 border-b border-border bg-surface/55 p-4 sm:grid-cols-2 lg:grid-cols-[minmax(12rem,1fr)_10rem_10rem_auto]">
            ...
            <Select
              value={filtreStatut}
              onValueChange={(value) => {
                setFiltreStatut(value as typeof filtreStatut);
                setPage(1);
              }}
            >
            ...
            <Button
              ...
              className="justify-self-start text-xs lg:justify-self-end"
            >
              {tCommon("reset")}
            </Button>
          </div>
...
function StatTrendCard({
  ...
  const toneStyles: Record<typeof tone, string> = {
    primary:
      "bg-teal-600 text-white shadow transition-colors hover:bg-teal-700",
    warning:
      "bg-teal-600 text-white shadow transition-colors hover:bg-teal-700",
    success:
      "bg-teal-600 text-white shadow transition-colors hover:bg-teal-700",
  ...
  return (
    <div className="flex items-center gap-3 rounded-xl border border-border bg-card p-4 shadow-soft">
      ...
      <div className="min-w-0">
        ...
        <p className="truncate text-xs text-muted-foreground">{label}</p>
        {delta !== undefined && (
          <p className={cn("mt-0.5 text-[11px] font-semibold", deltaStyle)}>
            {deltaTexte} {tCommon("thisWeek")}
          </p>
        )}
      </div>
    </div>
...
const CLAIM_STATUTS: ClaimAdminFull["status"][] = [
  "NOUVEAU",
  "EN_COURS",
  "RESOLU",
  "REJETE",
...
function ClaimStatusSelect({
  ...
  async function updateStatus(
    ...
    setPending(true);
    try {
      const result = await apiFetch<{ emailSent?: boolean | null }>(
        `/claims/${claim.id}/status`,
        {
          method: "PATCH",
          body: JSON.stringify({
            status,
    ...
    <>
      ...
      >
        <form
          onSubmit={(event) => {
            event.preventDefault();
            void updateStatus("REJETE", reason);
          }}
        >
          ...
          <label
            htmlFor={`claim-rejection-reason-${claim.id}`}
          ...
          <Textarea
            ...
            aria-describedby={
              reasonError ? `claim-rejection-error-${claim.id}` : undefined
            }
          />
          {reasonError && (
            <p
              id={`claim-rejection-error-${claim.id}`}
...
function ReclamationsAdmin() {
  ...
  const {
    ...
    error,
    refetch,
  } = useApiList<ClaimAdminFull>("/claims?lang=fr&pageSize=100");
  const [recherche, setRecherche] = useState("");
  const [filtreStatut, setFiltreStatut] = useState<
    "all" | ClaimAdminFull["status"]
  >("all");
  const [filtreCategorie, setFiltreCategorie] = useState("all");
  ...
  const categories = Array.from(
    new Set(claims.map((claim) => claim.claimType)),
  ).filter(Boolean);
  ...
  const resultat = claims.filter((claim) => {
    const termes =
      `${claim.firstname} ${claim.lastname} ${claim.concernedOperator} ${claim.email} ${claim.telephone ?? ""}`.toLocaleLowerCase(
        "fr",
      );
    const correspondRecherche = termes.includes(
      recherche.trim().toLocaleLowerCase("fr"),
    );
    const correspondStatut =
      filtreStatut === "all" || claim.status === filtreStatut;
    const correspondCategorie =
      filtreCategorie === "all" || claim.claimType === filtreCategorie;
  ...
  const nouvellesCetteSemaine = claims.filter((claim) =>
    depuisMoinsDuneSemaine(claim.createdAt),
  ).length;
  ...
  function basculerSelectionPage() {
    ...
    setSelection((prev) => {
      const suivant = new Set(prev);
      idsPage.forEach((id) =>
        tousSelectionnes ? suivant.delete(id) : suivant.add(id),
      );
      return suivant;
    });
  }
  ...
  async function supprimerSelection() {
    ...
    if (
      !(await confirm(t("confirmDeleteSelection", { count: selection.size })))
    )
      return;
  ...
  async function supprimerUne(claim: ClaimAdminFull) {
    if (
      !(await confirm(
        t("confirmDeleteOne", { name: `${claim.firstname} ${claim.lastname}` }),
      ))
    )
      return;
    try {
    ...
    <section className="mt-8 space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        ...
        <StatTrendCard
          ...
          label={t("stats.inProgress")}
          delta={
            enCours.filter((c) => depuisMoinsDuneSemaine(c.updatedAt)).length
          }
        />
        <StatTrendCard
          icon={CheckCircle2}
          tone="success"
          value={resolues.length}
          label={t("stats.resolved")}
          delta={
            resolues.filter((c) => depuisMoinsDuneSemaine(c.updatedAt)).length
          }
        />
        <StatTrendCard
          icon={X}
          ...
          delta={
            rejetees.filter((c) => depuisMoinsDuneSemaine(c.updatedAt)).length
          }
        />
      </div>
              ...
              <tr>
                <th className="w-10 px-5 py-3">
                  <input
                    ...
                    checked={
                      resultatPage.length > 0 &&
                      resultatPage.every((c) => selection.has(c.id))
                    }
                  ...
                  >
                    <td className="px-5 py-3.5">
                      <input
                        ...
                        onChange={() => basculerSelection(claim.id)}
                    ...
                    <td className="px-4 py-3.5">
                      <StatutBadge statut={claim.status} />
                    </td>
                    <td className="px-5 py-3.5 text-right">
                      <div className="flex items-center justify-end gap-1">
                        <button
                          type="button"
                          onClick={() =>
                            setOuvert((v) => (v === claim.id ? null : claim.id))
                          }
                              ...
                              {claim.attachments.map((piece) => (
                                <li key={piece.id}>
                                  <a
                                    ...
                                    target="_blank"
                                    rel="noreferrer"
                                    className="inline-flex items-center gap-1.5 rounded-md border border-border bg-card px-2.5 py-1 text-xs font-medium text-primary hover:underline"
                                  >
                                    <Paperclip
                                      className="size-3.5"
                                      aria-hidden
...
function AuditAdmin() {
  ...
  const {
    ...
    loading,
    error,
  } = useApiList<AuditLogEntry>("/audit-logs?pageSize=100");
  ...
  return (
    <TableauAdmin
      ...
      lignes={logs.map((j) => [
        j.actor?.email ?? t("system"),
        j.action,
        j.entity,
        formaterDate(j.createdAt),
      ])}
    />
  );
}
...
function AppelsOffresAdmin() {
  ...
  const {
    ...
    error,
    refetch,
  } = useApiList<TendersCallAdmin>("/tenders?lang=fr&pageSize=100");
  const { data: categories } = useApiOne<{ id: number; name: string }[]>(
    "/tender-categories?lang=fr",
  );
  const [showForm, setShowForm] = useState(false);
  ...
  const [recherche, setRecherche] = useState("");
  const [filtreStatut, setFiltreStatut] = useState<
    "all" | (typeof TENDER_STATUTS)[number]
  >("all");
  const [filtreCategorie, setFiltreCategorie] = useState("all");
  const [dateDebut, setDateDebut] = useState("");
  const [dateFin, setDateFin] = useState("");
  ...
  const categoriesNoms = Array.from(
    new Set(tenders.map((tender) => tender.category.name)),
  );
  ...
  const resultat = tenders.filter((tender) => {
    const correspondRecherche = `${tender.name} ${tender.code}`
      .toLocaleLowerCase("fr")
      .includes(recherche.trim().toLocaleLowerCase("fr"));
    const correspondStatut =
      filtreStatut === "all" || tender.status === filtreStatut;
    const correspondCategorie =
      filtreCategorie === "all" || tender.category.name === filtreCategorie;
  ...
  const nouveauxCetteSemaine = tenders.filter((tender) =>
    depuisMoinsDuneSemaine(tender.publicationDate),
  ).length;
    ...
    <section className="mt-8 space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        ...
        <StatTrendCard
          icon={Clock3}
          tone="warning"
          value={clotures.length}
          label={t("stats.closed")}
        />
        <StatTrendCard
          icon={X}
          tone="destructive"
          value={annules.length}
        ...
        >
          ...
          <div className="grid gap-2">
            ...
            <Input
              ...
              type="date"
              required
              defaultValue={editing?.publicationDate?.slice(0, 10)}
            />
          </div>
          <div className="grid gap-2">
            ...
            <Input
              ...
              type="date"
              required
              defaultValue={editing?.limitDate?.slice(0, 10)}
            />
          </div>
          <div className="grid gap-2">
            ...
            <Input
              ...
              name="budget"
              type="number"
              placeholder={t("fields.budgetOptional")}
          ...
          <div className="grid gap-2 sm:col-span-2">
            <Label htmlFor="ao-dossier">
              {t("fields.file", {
                hint: editing
                  ? t("fields.fileHintKeep")
                  : t("fields.fileHintRequired"),
              })}
            </Label>
            <Input
              id="ao-dossier"
              ...
              type="file"
              accept=".pdf"
              required={!editing || !editing.fileUrl}
            />
          </div>
          {formError && (
            <p className="text-sm text-destructive sm:col-span-2" role="alert">
      ...
      <div className="overflow-hidden rounded-2xl border border-border bg-card shadow-card">
        ...
        <TableauAdmin
          ...
          lignes={resultatPage.map((tender) => [
            ...
            <RowActions
              key={tender.id}
              onEdit={() => ouvrirEdition(tender)}
              onDelete={() => supprimer(tender)}
            />,
          ])}
        />
        {resultatPage.length === 0 && (
...
function CarrieresAdmin() {
  ...
  const {
    ...
    error,
    refetch,
  } = useApiList<CareerAdmin>("/careers?lang=fr&pageSize=100");
  const { data: categories } = useApiOne<{ id: number; name: string }[]>(
    "/career-categories?lang=fr",
  );
  const [showForm, setShowForm] = useState(false);
  ...
  const categoriesNoms = Array.from(
    new Set(careers.map((c) => c.category.name)),
  );
  ...
  const resultat = careers.filter((c) => {
    const correspondRecherche = `${c.name} ${c.code} ${c.departement}`
      .toLocaleLowerCase("fr")
      .includes(recherche.trim().toLocaleLowerCase("fr"));
    const enCours = c.limitDate.slice(0, 10) >= aujourdhui;
    const correspondStatut =
      filtreStatut === "all" ||
      (filtreStatut === "en_cours" ? enCours : !enCours);
    const correspondCategorie =
      filtreCategorie === "all" || c.category.name === filtreCategorie;
  ...
  const enCoursListe = careers.filter(
    (c) => c.limitDate.slice(0, 10) >= aujourdhui,
  );
  const cloturesListe = careers.filter(
    (c) => c.limitDate.slice(0, 10) < aujourdhui,
  );
  const totalCandidatures = careers.reduce(
    (somme, c) => somme + c.candidatCount,
    0,
  );
  const nouveauxCetteSemaine = careers.filter((c) =>
    depuisMoinsDuneSemaine(c.publicationDate),
  ).length;
  ...
  async function soumettre(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (form.get("locationFr"))
      body.append("locationFr", String(form.get("locationFr")));
    ...
    <section className="mt-8 space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        ...
        <StatTrendCard
          icon={Clock3}
          tone="warning"
          value={enCoursListe.length}
          label={t("stats.inProgress")}
        />
        <StatTrendCard
          icon={CheckCircle2}
          tone="success"
          value={cloturesListe.length}
          label={t("stats.closed")}
        />
        <StatTrendCard
          icon={Users}
        ...
        >
          ...
          <div className="grid gap-2">
            ...
            <Input
              ...
              name="departementFr"
              required
              defaultValue={editing?.departement}
            />
          </div>
          <div className="grid gap-2">
          ...
          <div className="grid gap-2">
            ...
            <Input
              id="cr-salary"
              name="salary"
              placeholder={t("fields.salaryOptional")}
      ...
      <div className="overflow-hidden rounded-2xl border border-border bg-card shadow-card">
        ...
        <TableauAdmin
          ...
          lignes={resultatPage.map((c) => [
            ...
            c.candidatCount,
            c.isNew ? (
              <Puce key={c.id} label={t("isNew")} tone="info" />
            ) : (
              <Puce key={c.id} label={t("published")} tone="success" />
            ),
            <RowActions
              key={c.id}
              onEdit={() => ouvrirEdition(c)}
...
function RapportsMarchesAdmin() {
  const {
    data: reports,
    loading,
    error,
    refetch,
  } = useApiList<RapportMarcheAdmin>(
    "/statistics/reports?lang=fr&pageSize=100&ordering=-createdAt",
  );
  const confirm = useConfirm();
  const [recherche, setRecherche] = useState("");
  const [secteur, setSecteur] = useState("all");
  const [annee, setAnnee] = useState("all");
  const [editing, setEditing] = useState<RapportMarcheAdmin | "new" | null>(
    null,
  );
  const [submitting, setSubmitting] = useState(false);
  ...
  const filtered = reports.filter(
    (r) =>
      (secteur === "all" || r.sector === secteur) &&
      (annee === "all" || r.year === Number(annee)) &&
      r.title
        .toLocaleLowerCase("fr")
        .includes(recherche.trim().toLocaleLowerCase("fr")),
  );
  const categoriesRapports = [
    { value: "TELECOM", label: "Télécom" },
    { value: "TARIFAIRE", label: "Tarifaire" },
  ...
  async function enregistrer(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      if (editing === "new")
        await apiFetch("/statistics/reports", { method: "POST", body });
      else if (editing)
        await apiFetch("/statistics/reports/" + editing.id, {
          method: "PATCH",
          body,
        });
      toast.success(
        editing === "new" ? "Publication créée." : "Publication mise à jour.",
      );
      setEditing(null);
      refetch();
    } catch (err) {
  ...
  async function supprimer(report: RapportMarcheAdmin) {
    if (!(await confirm("Supprimer « " + report.title + " » ?"))) return;
    try {
      await apiFetch("/statistics/reports/" + report.id, { method: "DELETE" });
  ...
  return (
    <section className="rounded-xl border border-border bg-card p-5 shadow-soft lg:p-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <h2 className="font-heading text-lg font-semibold">
            Publications des indicateurs des marchés
          </h2>
          <p className="mt-1 text-xs text-muted-foreground">
            Créez, modifiez, remplacez le PDF ou retirez une publication.
          </p>
        </div>
        {editing === null && (
      ...
      <div className="mt-5 grid gap-3 sm:grid-cols-3">
        {categoriesRapports.map((categorie) => {
          const count = reports.filter(
            (r) => r.sector === categorie.value,
          ).length;
          const active = secteur === categorie.value;
          return (
            <button
              key={categorie.value}
              type="button"
              onClick={() => setSecteur(active ? "all" : categorie.value)}
              className={
                active
                  ? "rounded-xl border border-primary bg-primary p-4 text-left text-primary-foreground shadow"
                  : "rounded-xl border border-border bg-surface p-4 text-left transition hover:border-primary/40"
              }
            >
              ...
              <span
                className={
                  active
                    ...
                    : "mt-1 block text-2xl font-bold text-primary"
                }
              >
                {count}
              </span>
              ...
              >
                publications administrables
              </span>
            </button>
          );
        })}
      </div>
      <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-[minmax(14rem,1fr)_12rem_9rem]">
        <div className="relative">
          <Search
        ...
        <Select value={secteur} onValueChange={setSecteur}>
          ...
          <SelectContent>
            ...
            <SelectItem value="TARIFAIRE">Tarifaire</SelectItem>
      ...
      {editing !== null && (
        <form
          ...
          className="mt-5 grid gap-4 rounded-xl border border-primary/15 bg-surface p-5 sm:grid-cols-2"
        >
          <div className="grid gap-2 sm:col-span-2">
          ...
          <div className="grid gap-2">
            <Label htmlFor="report-sector">Catégorie *</Label>
            <select
              id="report-sector"
              name="sector"
              required
              defaultValue={current?.sector ?? "TELECOM"}
          ...
          <div className="grid gap-2">
            <Label htmlFor="report-quarter">Trimestre</Label>
            <Input
              id="report-quarter"
              name="quarter"
              type="number"
              min={1}
              max={4}
              defaultValue={current?.quarter ?? undefined}
            />
          </div>
          <div className="grid gap-2">
            <Label htmlFor="report-file">
              Document PDF {current ? "(laisser vide pour conserver)" : "*"}
            </Label>
            <Input
              id="report-file"
              name="file"
...
function StatistiquesAdmin() {
  ...
  const {
    ...
    loading,
    error,
  } = useApiOne<SectorOverview>("/statistics/overview?lang=fr");
    ...
    <div className="mt-8 grid gap-6">
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <div className="rounded-xl border border-border bg-card p-5 shadow-soft">
          <p className="text-xs font-semibold tracking-wide text-muted-foreground uppercase">
            {t("kpi.mobileSubscribers")}
          </p>
          <p className="mt-3 font-heading text-2xl font-bold text-primary">
            {overview.kpis?.subscribersMillion != null
              ? `${overview.kpis.subscribersMillion} M`
              : "—"}
          </p>
        </div>
        <div className="rounded-xl border border-border bg-card p-5 shadow-soft">
          <p className="text-xs font-semibold tracking-wide text-muted-foreground uppercase">
            {t("kpi.penetrationRate")}
          </p>
          <p className="mt-3 font-heading text-2xl font-bold text-primary">
            {overview.kpis?.penetrationRate != null
              ? `${overview.kpis.penetrationRate} %`
              : "—"}
          </p>
        </div>
        <div className="rounded-xl border border-border bg-card p-5 shadow-soft">
          <p className="text-xs font-semibold tracking-wide text-muted-foreground uppercase">
            {t("kpi.activeOperators")}
          </p>
          <p className="mt-3 font-heading text-2xl font-bold text-primary">
            {overview.kpis?.activeOperators ?? "—"}
          </p>
        </div>
        <div className="rounded-xl border border-border bg-card p-5 shadow-soft">
          <p className="text-xs font-semibold tracking-wide text-muted-foreground uppercase">
          ...
          <p className="mt-3 font-heading text-2xl font-bold text-primary">
            {overview.kpis?.active4GSites != null
              ? overview.kpis.active4GSites.toLocaleString("fr-FR")
              : "—"}
          </p>
        </div>
      </div>
...
function PointStatistiqueForm({
  ...
  const t = useTranslations("admin.statistics");
  const tCommon = useTranslations("admin.common");
  const [periodeType, setPeriodeType] = useState<"month" | "quarter">(
    entry?.quarter ? "quarter" : "month",
  );
  const [submitting, setSubmitting] = useState(false);
  const [formError, setFormError] = useState("");
  ...
  async function enregistrer(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      if (entry) {
        await apiFetch(`/statistics/entries/${entry.id}`, {
          method: "PATCH",
          body: JSON.stringify(body),
        });
        toast.success(t("form.updated"));
      } else {
        await apiFetch("/statistics/entries", {
          method: "POST",
          body: JSON.stringify(body),
        });
  ...
  return (
    <form
      onSubmit={enregistrer}
      className="grid gap-4 rounded-xl border border-primary/15 bg-surface p-5 shadow-soft"
    >
      <div className="grid gap-4 sm:grid-cols-3">
        ...
        <div className="grid gap-2">
          ...
          <Select
            value={periodeType}
            onValueChange={(v) => setPeriodeType(v as "month" | "quarter")}
          >
            <SelectTrigger>
              <SelectValue />
            </SelectTrigger>
        ...
        {periodeType === "month" ? (
          <div className="grid gap-2">
            ...
            <Select
              name="month"
              defaultValue={entry?.month ? String(entry.month) : undefined}
              required
            >
          ...
          <div className="grid gap-2">
            ...
            <Select
              name="quarter"
              defaultValue={entry?.quarter ? String(entry.quarter) : undefined}
              required
            >
      ...
      {periodeType === "month" ? (
        <div className="grid gap-4 sm:grid-cols-3">
          <div className="grid gap-2">
            <Label htmlFor="stat-subscribersMillion">
              {t("form.mobileSubscribers")}
            </Label>
            <Input
              id="stat-subscribersMillion"
              name="subscribersMillion"
              type="number"
              step="0.1"
              min={0}
              defaultValue={entry?.subscribersMillion ?? undefined}
            />
          </div>
          <div className="grid gap-2">
            <Label htmlFor="stat-penetrationRate">
              {t("form.mobilePenetration")}
            </Label>
            <Input
              id="stat-penetrationRate"
              name="penetrationRate"
              type="number"
              step="0.1"
              min={0}
              defaultValue={entry?.penetrationRate ?? undefined}
            />
          </div>
          <div className="grid gap-2">
            <Label htmlFor="stat-internetSubscribersMillion">
              {t("form.internetSubscribers")}
            </Label>
            <Input
              id="stat-internetSubscribersMillion"
              name="internetSubscribersMillion"
              type="number"
              step="0.1"
              min={0}
              defaultValue={entry?.internetSubscribersMillion ?? undefined}
            />
          </div>
          <div className="grid gap-2">
            <Label htmlFor="stat-internetPenetrationRate">
              {t("form.internetPenetration")}
            </Label>
            <Input
              id="stat-internetPenetrationRate"
              name="internetPenetrationRate"
              type="number"
              step="0.1"
              min={0}
              defaultValue={entry?.internetPenetrationRate ?? undefined}
            />
          </div>
          <div className="grid gap-2">
            <Label htmlFor="stat-mobileMoneyPenetrationRate">
              {t("form.mobileMoneyPenetration")}
            </Label>
            <Input
              id="stat-mobileMoneyPenetrationRate"
              name="mobileMoneyPenetrationRate"
              type="number"
              step="0.1"
              min={0}
              defaultValue={entry?.mobileMoneyPenetrationRate ?? undefined}
            />
          </div>
          <div className="grid gap-2">
            <Label htmlFor="stat-salariedJobs">{t("form.salariedJobs")}</Label>
            <Input
              id="stat-salariedJobs"
              name="salariedJobs"
              type="number"
              min={0}
              defaultValue={entry?.salariedJobs ?? undefined}
            />
          </div>
          <div className="grid gap-2">
            <Label htmlFor="stat-activeOperators">
              {t("form.activeOperators")}
            </Label>
            <Input
              id="stat-activeOperators"
              name="activeOperators"
              type="number"
              min={0}
              defaultValue={entry?.activeOperators ?? undefined}
            />
          </div>
          <div className="grid gap-2">
            <Label htmlFor="stat-active4GSites">
              {t("form.active4GSites")}
            </Label>
            <Input
              id="stat-active4GSites"
              name="active4GSites"
              type="number"
              min={0}
              defaultValue={entry?.active4GSites ?? undefined}
            />
          </div>
        </div>
      ) : (
        <div className="grid gap-2 sm:max-w-xs">
          ...
          <Input
            id="stat-revenueBillionGNF"
            name="revenueBillionGNF"
            type="number"
            step="0.1"
            min={0}
            defaultValue={entry?.revenueBillionGNF ?? undefined}
          />
        </div>
      )}
...
function PointsStatistiquesAdmin() {
  ...
  const {
    ...
    error,
    refetch,
  } = useApiOne<SectorStatisticEntry[]>("/statistics/entries");
  const [editing, setEditing] = useState<SectorStatisticEntry | "new" | null>(
    null,
  );
  const [recherche, setRecherche] = useState("");
  const [typePeriode, setTypePeriode] = useState<"all" | "month" | "quarter">(
    "all",
  );
  const confirm = useConfirm();
  const entreesFiltrees = (entries ?? []).filter((entry) => {
    const periode = entry.month
      ? MOIS_COURTS[entry.month - 1] + " " + entry.year
  ...
  if (loading)
    return (
      <Loader className="flex justify-center py-8" label={tCommon("loading")} />
    );
  if (error) return <p className="text-sm text-destructive">{error}</p>;
  ...
  return (
    <div className="rounded-xl border border-border bg-card p-5 shadow-soft lg:p-6">
      ...
      <div className="mt-5 grid gap-3 sm:grid-cols-[minmax(15rem,1fr)_12rem]">
        <div className="relative">
          ...
          <Input
            ...
            className="pl-9"
            placeholder="Rechercher une période ou une année"
          />
        </div>
        <Select
          value={typePeriode}
          onValueChange={(value) => setTypePeriode(value as typeof typePeriode)}
        >
          <SelectTrigger>
            <SelectValue />
          </SelectTrigger>
          <SelectContent>
            ...
            <SelectItem value="month">Mensuelles</SelectItem>
            <SelectItem value="quarter">Trimestrielles</SelectItem>
          </SelectContent>
        </Select>
      </div>
      ...
      <TableauAdmin
        codeColumn={false}
        colonnes={[
          t("entries.columns.period"),
          t("entries.columns.mobileSubscribers"),
          t("entries.columns.mobilePenetration"),
          t("entries.columns.internetSubscribers"),
          t("entries.columns.mobileMoney"),
          t("entries.columns.jobs"),
          tCommon("actions"),
        ]}
        lignes={entreesFiltrees.map((e) => [
          e.month
            ? `${MOIS_COURTS[e.month - 1]} ${e.year}`
            : `T${e.quarter} ${e.year}`,
          e.subscribersMillion != null ? `${e.subscribersMillion} M` : "—",
          e.penetrationRate != null ? `${e.penetrationRate} %` : "—",
          e.internetSubscribersMillion != null
            ? `${e.internetSubscribersMillion} M`
            : "—",
          e.mobileMoneyPenetrationRate != null
            ? `${e.mobileMoneyPenetrationRate} %`
            : "—",
      ...
      {entreesFiltrees.length === 0 && (
        <p className="mt-4 text-sm text-muted-foreground">
          {recherche || typePeriode !== "all"
            ? "Aucune statistique ne correspond aux filtres."
            : t("entries.noResults")}
        </p>
      )}
    </div>
...
function ConfigurationAdmin() {
  ...
  const {
    ...
    error,
    refetch,
  } = useApiOne<SiteConfig>("/site-config");
...
function ConfigurationInner({
  ...
  const [colors, setColors] = useState<Required<ThemeColors>>({
    ...DEFAULT_THEME_COLORS,
    ...initialValue.themeColors,
  });
  ...
  async function enregistrerApparence() {
    ...
    try {
      await apiFetch("/site-config", {
        method: "PATCH",
        body: JSON.stringify({ logoKey: logoKey || null, themeColors: colors }),
      });
      toast.success(t("saved"));
      onSaved();
      window.dispatchEvent(new Event("arpt:site-config-updated"));
  ...
  return (
    <form
      onSubmit={enregistrer}
      className="mt-8 grid max-w-2xl gap-5 rounded-xl border border-border bg-card p-6"
    >
      <div className="grid gap-4 sm:grid-cols-2">
        <div className="grid gap-2">
          ...
          <Input
            id="cfg-phone"
            name="phone"
            defaultValue={initialValue.contactInfo?.phone ?? ""}
          />
        </div>
        <div className="grid gap-2">
          ...
          <Input
            id="cfg-phone2"
            name="phoneSecondary"
            defaultValue={initialValue.contactInfo?.phoneSecondary ?? ""}
          />
        </div>
        <div className="grid gap-2">
          ...
          <Input
            ...
            name="email"
            type="email"
            defaultValue={initialValue.contactInfo?.email ?? ""}
          />
        </div>
        <div className="grid gap-2">
          ...
          <Input
            id="cfg-address"
            name="address"
            defaultValue={initialValue.contactInfo?.address?.fr ?? ""}
          />
        </div>
        <div className="grid gap-2">
          ...
          <Input
            id="cfg-hours"
            name="hours"
            defaultValue={initialValue.contactInfo?.hours?.fr ?? ""}
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <div className="grid gap-2">
          ...
          <Input
            id="cfg-fb"
            name="facebook"
            defaultValue={initialValue.socialLinks?.facebook ?? ""}
        ...
        <div className="grid gap-2">
          ...
          <Input
            id="cfg-tw"
            name="twitter"
            defaultValue={initialValue.socialLinks?.twitter ?? ""}
        ...
        <div className="grid gap-2">
          ...
          <Input
            id="cfg-li"
            name="linkedin"
            defaultValue={initialValue.socialLinks?.linkedin ?? ""}
        ...
        <div className="grid gap-2">
          ...
          <Input
            id="cfg-yt"
            name="youtube"
            defaultValue={initialValue.socialLinks?.youtube ?? ""}
      ...
      <div className="grid gap-2">
        ...
        <Input
          id="cfg-footer"
          name="footerText"
          defaultValue={initialValue.footerText?.fr ?? ""}
        />
      </div>
      ...
      <div className="mt-2 border-t border-border pt-5">
        ...
        <div className="mt-6">
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            {tAppearance("colorsHint")}
          </p>
          <div className="mt-3 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {THEME_COLOR_FIELDS.map((field) => (
              <ColorField
                key={field}
                label={tAppearance(field)}
                value={colors[field]}
        ...
        <div className="mt-5">
          ...
          >
            ...
            <div className="p-4">
              ...
              >
                {tAppearance("textPrimary")}
              </p>
              <p
                className="mt-1 text-xs"
                style={{ color: colors.textSecondary }}
              >
                {tAppearance("textSecondary")}
              </p>
              <div className="mt-3 flex flex-wrap gap-2">
                ...
                <span
                  className="rounded-full px-2.5 py-1 text-xs font-semibold"
                  style={{ background: colors.warning, color: "#fff" }}
                >
                  {tAppearance("warning")}
                </span>
                <span
...
function ImageField({
  ...
  async function envoyer(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      const res = await apiFetch<{ url: string }>("/site-config/upload-image", {
        method: "POST",
        body,
      });
      onChange(res.url);
      ...
      toast.error(
        err instanceof ApiError ? err.message : t("imageUploadError"),
      );
    } finally {
      setUploading(false);
      event.target.value = "";
...
function FileField({
  ...
  async function envoyer(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      const res = await apiFetch<{ url: string }>("/site-config/upload-file", {
        method: "POST",
        body,
      });
      onChange(res.url);
...
function BlockFieldInput({
  ...
  if (field.type === "select") {
    return (
      <Select
        value={strValue || (field.options?.[0]?.value ?? "")}
...
function ObjectBlockAdmin({
  ...
  const tCommon = useTranslations("admin.common");
  const { data, loading } = useContentBlockRaw<BlockItem>(
    blockKey,
    defaultValue,
  );
  if (loading)
...
function ObjectBlockInner({
  ...
  async function enregistrer() {
    setSubmitting(true);
    try {
      await apiFetch(`/content-blocks/${blockKey}`, {
        method: "PUT",
...
function ListBlockInner({
  ...
  function ajouter() {
    const nouveau =
      emptyItem ??
      Object.fromEntries(fields.map((f) => [f.name, emptyBlockValue(f)]));
  ...
  function modifier(index: number, name: string, v: BlockFieldValue) {
    setItems((prev) =>
      prev.map((it, i) => (i === index ? { ...it, [name]: v } : it)),
    );
  }
  ...
  async function enregistrer() {
    ...
    const nonVides = items.filter((it) =>
      Object.values(it).some(blockValueIsFilled),
    );
    setSubmitting(true);
    try {
      await apiFetch(`/content-blocks/${blockKey}`, {
        method: "PUT",
        body: JSON.stringify({ value: nonVides }),
      });
      toast.success(tCommon("savedContent"));
      setItems(nonVides);
    } catch (err) {
              ...
              {fields.map((f) => (
                ...
                >
                  ...
                  <BlockFieldInput
                    field={f}
                    value={item[f.name] ?? ""}
                    onChange={(v) => modifier(index, f.name, v)}
                  />
                </div>
              ))}
            </div>
...
const ICONE_OPTIONS_ACCUEIL = [
  ...
  { value: "carrieres", label: "Mallette (offres d’emploi)" },
...
function ConsumerRightsDocumentAdmin() {
  ...
  const { data, loading, refetch } = useApiOne<{
    fileUrl: string | null;
  ...
  return (
    <div className="mt-8">
      <div className="rounded-2xl border border-border bg-card p-5 shadow-soft sm:p-6">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="font-heading text-xs font-semibold tracking-[0.15em] text-primary uppercase">
                {t("editorTitle")}
              </p>
          ...
          <div className="grid gap-1.5 sm:min-w-72">
            <Label htmlFor="public-page-select" className="text-xs">
              {t("pageLabel")}
            </Label>
            <Select
              value={pageSelectionnee}
              onValueChange={setPageSelectionnee}
            >
              <SelectTrigger id="public-page-select" className="bg-surface">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                {pages.map((page) => (
      ...
      <div className="mt-6">
                ...
                defaultItems={[
                  {
                    to: "/services",
                    label: {
                      fr: "Démarches et services",
                      en: "Procedures and services",
                      ar: "الإجراءات والخدمات",
                    },
                    icone: "services",
                    texte: {
                      fr: "Licences, homologations, fréquences",
                      en: "Licenses, approvals, frequencies",
                      ar: "التراخيص، الاعتمادات، الترددات",
                    },
                  },
                  {
                    to: "/equipements",
                    label: {
                      fr: "Équipements homologués",
                      en: "Approved equipment",
                      ar: "المعدات المعتمدة",
                    },
                    icone: "equipements",
                    texte: {
                      fr: "Vérifier un terminal agréé",
                      en: "Check an approved device",
                      ar: "التحقق من جهاز معتمد",
                    },
                  },
                  {
                    to: "/appels-offres",
                    ...
                    icone: "marches",
                    texte: {
                      fr: "Consulter les marchés en cours",
                      en: "Browse ongoing contracts",
                      ar: "تصفح الصفقات الجارية",
                    },
                  },
                  {
                    to: "/carrieres",
                    ...
                    icone: "carrieres",
                    texte: {
                      fr: "Rejoindre l'Autorité",
                      en: "Join the Authority",
                      ar: "انضم إلى الهيئة",
                    },
                  },
                  {
                    ...
                    icone: "reclamations",
                    texte: {
                      fr: "Signaler un litige opérateur",
                      en: "Report a dispute with an operator",
                      ar: "الإبلاغ عن نزاع مع مشغل",
                    },
                  },
                  {
                    to: "/indicateurs-des-marches",
                    label: {
                      fr: "Indicateurs des marchés",
                      en: "Market indicators",
                      ar: "مؤشرات السوق",
                    },
                    icone: "statistiques",
                    texte: {
                      fr: "Observatoires officiels du secteur",
                      en: "Official sector observatories",
                      ar: "المراصد الرسمية للقطاع",
                    },
                  },
                ]}
              />
                ...
                defaultItems={[
                  {
                    ...
                    titre: {
                      fr: "Participation de l'ARPT à une conférence internationale",
                      en: "ARPT's participation in an international conference",
                      ar: "مشاركة الهيئة في مؤتمر دولي",
                    },
                  },
                  {
                    ...
                    titre: {
                      fr: "Visite officielle à l'ARPT",
                      en: "Official visit to ARPT",
                      ar: "زيارة رسمية إلى الهيئة",
                    },
                  },
                ]}
              />
            ...
            <div className="mt-3 space-y-5">
              <ObjectBlockAdmin
                ...
                defaultValue={{
                  ...
                  titre: {
                    fr: "Une institution au service d'un marché numérique équitable",
                    en: "An institution serving a fair digital market",
                    ar: "مؤسسة في خدمة سوق رقمي عادل",
                  },
                  description: {
                    fr: "L'ARPT est l'autorité administrative indépendante chargée de la régulation des secteurs des postes et des télécommunications en République de Guinée.",
                    en: "ARPT is the independent administrative authority responsible for regulating the postal and telecommunications sectors in the Republic of Guinea.",
                    ar: "الهيئة هي السلطة الإدارية المستقلة المكلفة بتنظيم قطاعي البريد والاتصالات في جمهورية غينيا.",
                  },
                }}
              />
              <ObjectBlockAdmin
                ...
                fields={[
                  ...
                  {
                    name: "phrase",
                    label: "Phrase affichée sur l'image",
                    type: "text",
                    translatable: true,
                  },
                ]}
                defaultValue={{
                  image: "/images/hero-arpt.jpg",
                  phrase: {
                    fr: "Réguler les infrastructures qui connectent la Guinée",
                    en: "Regulating the infrastructure that connects Guinea",
                    ar: "تنظيم البنية التحتية التي تربط غينيا",
                  },
                }}
              />
              <ObjectBlockAdmin
                ...
                fields={[
                  {
                    ...
                    label: "Icône",
                    type: "select",
                    options: ICONE_OPTIONS_AUTORITE,
                  },
                  {
                    name: "titre",
                    label: "Titre",
                ...
                defaultItems={[
                  {
                    icone: "scale",
                    titre: {
                      fr: "Garantir une concurrence loyale",
                      en: "Ensure fair competition",
                      ar: "ضمان منافسة نزيهة",
                    },
                    texte: {
                      fr: "Surveiller les marchés, encadrer les tarifs d'interconnexion et prévenir les pratiques anticoncurrentielles.",
                      en: "Monitor markets, regulate interconnection tariffs and prevent anti-competitive practices.",
                      ar: "مراقبة الأسواق، وتأطير تعرفات الربط البيني، ومنع الممارسات المنافية للمنافسة.",
                    },
                  },
                  {
                    icone: "radio",
                    titre: {
                      fr: "Gérer les ressources rares",
                      en: "Manage scarce resources",
                      ar: "إدارة الموارد النادرة",
                    },
                    texte: {
                      fr: "Planifier et attribuer le spectre radioélectrique ainsi que les ressources en numérotation.",
                      en: "Plan and allocate the radio spectrum as well as numbering resources.",
                      ar: "تخطيط وتوزيع الطيف الترددي وموارد الترقيم.",
                    },
                  },
                  {
                    icone: "users",
                    titre: {
                      fr: "Protéger les consommateurs",
                      en: "Protect consumers",
                      ar: "حماية المستهلكين",
                    },
                    texte: {
                      fr: "Traiter les réclamations, contrôler la qualité de service et informer les usagers de leurs droits.",
                      en: "Handle complaints, monitor service quality and inform users of their rights.",
                      ar: "معالجة الشكاوى، ومراقبة جودة الخدمة، وإعلام المستخدمين بحقوقهم.",
                    },
                  },
                  {
                    icone: "shield",
                    titre: {
                      fr: "Sécuriser le secteur",
                      en: "Secure the sector",
                      ar: "تأمين القطاع",
                    },
                    texte: {
                      fr: "Homologuer les équipements, contrôler les opérateurs et veiller au respect du cadre légal.",
                      en: "Approve equipment, oversee operators and ensure compliance with the legal framework.",
                      ar: "اعتماد المعدات، ومراقبة المشغلين، والسهر على احترام الإطار القانوني.",
                    },
                  },
                ]}
              />
                ...
                defaultItems={[
                  {
                    ...
                    texte: {
                      fr: "Pilotage stratégique, représentation institutionnelle et coordination de l'ensemble des directions.",
                      en: "Strategic direction, institutional representation and coordination of all departments.",
                      ar: "القيادة الاستراتيجية والتمثيل المؤسسي وتنسيق جميع المديريات.",
                    },
                  },
                  {
                    icone: "radio",
                    nom: {
                      fr: "Direction technique et du spectre",
                      en: "Technical and Spectrum Department",
                      ar: "المديرية التقنية والطيف",
                    },
                    texte: {
                      fr: "Planification des fréquences, contrôle du spectre et homologation des équipements radioélectriques.",
                      en: "Frequency planning, spectrum monitoring and approval of radio equipment.",
                      ar: "تخطيط الترددات، ومراقبة الطيف، واعتماد الأجهزة اللاسلكية.",
                    },
                  },
                  {
                    icone: "scale",
                    nom: {
                      ...
                      en: "Legal Affairs Department",
                      ar: "مديرية الشؤون القانونية",
                    },
                    texte: {
                      fr: "Élaboration des textes réglementaires, avis juridiques et suivi des contentieux sectoriels.",
                      en: "Drafting of regulatory texts, legal opinions and monitoring of sector disputes.",
                      ar: "إعداد النصوص التنظيمية، وإبداء الآراء القانونية، ومتابعة النزاعات القطاعية.",
                    },
                  },
                  {
                    icone: "users",
                    nom: {
                      fr: "Direction des consommateurs",
                      en: "Consumer Department",
                      ar: "مديرية المستهلكين",
                    },
                    texte: {
                      fr: "Traitement des réclamations des usagers et actions de sensibilisation sur leurs droits.",
                      en: "Handling user complaints and awareness actions on their rights.",
                      ar: "معالجة شكاوى المستخدمين وأنشطة التوعية بحقوقهم.",
                    },
                  },
                  {
                    icone: "trending",
                    nom: {
                      fr: "Direction de l'économie et des marchés",
                      en: "Economics and Markets Department",
                      ar: "مديرية الاقتصاد والأسواق",
                    },
                    texte: {
                      fr: "Analyse tarifaire, observatoire du secteur et surveillance de la concurrence entre opérateurs.",
                      en: "Tariff analysis, sector observatory and monitoring of competition between operators.",
                      ar: "تحليل التعرفات، ومرصد القطاع، ومراقبة المنافسة بين المشغلين.",
                    },
                  },
                  {
                    icone: "mail",
                    nom: {
                      fr: "Direction du secteur postal",
                      en: "Postal Sector Department",
                      ar: "مديرية القطاع البريدي",
                    },
                    texte: {
                      fr: "Régulation, autorisation et développement des activités postales et de courrier express.",
                      en: "Regulation, authorization and development of postal and express mail activities.",
                      ar: "تنظيم وترخيص وتطوير أنشطة البريد والبريد السريع.",
                    },
                  },
                ]}
              />
                ...
                defaultItems={[
                  {
                    annee: "2005",
                    texte: {
                      fr: "Création de l'Autorité de régulation du secteur.",
                      en: "Creation of the sector's regulatory Authority.",
                      ar: "إنشاء هيئة تنظيم القطاع.",
                    },
                  },
                  {
                    annee: "2015",
                    texte: {
                      fr: "Adoption de la loi L/2015/018/AN sur les télécommunications et les TIC.",
                      en: "Adoption of Law L/2015/018/AN on telecommunications and ICT.",
                      ar: "اعتماد القانون L/2015/018/AN المتعلق بالاتصالات وتكنولوجيا المعلومات.",
                    },
                  },
                  {
                    annee: "2016",
                    texte: {
                      fr: "Nouvelle organisation de l'ARPT par décret présidentiel.",
                      en: "New organization of ARPT by presidential decree.",
                      ar: "تنظيم جديد للهيئة بموجب مرسوم رئاسي.",
                    },
                  },
                  {
                    annee: "2026",
                    texte: {
                      fr: "Lancement du chantier d'attribution des fréquences 5G.",
                      en: "Launch of the 5G frequency allocation project.",
                      ar: "إطلاق ورش توزيع ترددات الجيل الخامس.",
                    },
                  },
                ]}
              />
            ...
            <div className="mt-3 space-y-5">
              <ObjectBlockAdmin
                ...
                defaultValue={{
                  ...
                  description: {
                    fr: "L'ARPT reçoit et instruit les litiges opposant les usagers aux opérateurs de télécommunications et aux opérateurs postaux.",
                    en: "ARPT receives and investigates disputes between users and telecommunications or postal operators.",
                    ar: "تتلقى الهيئة وتحقق في النزاعات بين المستخدمين ومشغلي الاتصالات والبريد.",
                  },
                }}
              />
              <ListBlockAdmin
                ...
                defaultItems={[
                  {
                    texte: {
                      fr: "Contactez d'abord le service client de votre opérateur et conservez la référence du dossier.",
                      en: "First contact your operator's customer service and keep the case reference.",
                      ar: "اتصل أولاً بخدمة عملاء مشغلك واحتفظ بمرجع الملف.",
                    },
                  },
                  {
                    texte: {
                      fr: "Si aucune réponse satisfaisante n'est apportée sous 30 jours, saisissez l'ARPT via ce formulaire.",
                      en: "If no satisfactory response is given within 30 days, contact ARPT via this form.",
                      ar: "إذا لم تحصل على رد مُرضٍ خلال 30 يوماً، توجه إلى الهيئة عبر هذه الاستمارة.",
                    },
                  },
                  {
                    texte: {
                      fr: "Un agent instruit votre dossier et vous informe de l'avancement depuis votre portail usager.",
                      en: "An agent processes your case and keeps you informed of its progress via your user portal.",
                      ar: "يتولى أحد الأعوان معالجة ملفك ويطلعك على تقدمه عبر بوابة المستخدم.",
                    },
                  },
                ]}
              />
              <ListBlockAdmin
                ...
                fields={[
                  ...
                  },
                  {
                    name: "serviceValues",
                    label:
                      "Services liés (identifiants séparés par des virgules)",
                    type: "text",
                  },
                ]}
                defaultItems={[
                  ...
                  {
                    value: "poste-guineenne",
                    label: {
                      fr: "La Poste Guinéenne",
                      en: "Guinean Post",
                      ar: "البريد الغيني",
                    },
                  },
                  {
                    value: "dhl-guinee",
                    label: {
                      fr: "DHL International Guinée",
                      en: "DHL International Guinea",
                      ar: "دي إتش إل غينيا",
                    },
                  },
                  {
              ...
              <ListBlockAdmin
                ...
                defaultItems={[
                  ...
                  {
                    value: "facturation",
                    label: { fr: "Facturation", en: "Billing", ar: "الفوترة" },
                  },
                  {
                    value: "reseau",
                    label: {
                      fr: "Réseau / couverture",
                      en: "Network / coverage",
                      ar: "الشبكة / التغطية",
                    },
                  },
              ...
              />
              <ListBlockAdmin
                blockKey="claims.services"
                title="Services concernés"
                description="Les problèmes et opérateurs utilisent ces identifiants pour leurs relations."
                itemLabel={(it) => blockLabel(it.label) || "Nouveau service"}
                fields={[
                  { name: "value", label: "Identifiant", type: "text" },
                  {
                    name: "label",
                    label: "Nom affiché",
                    type: "text",
              ...
              />
              <ListBlockAdmin
                blockKey="claims.locations"
                title="Régions, préfectures et communes"
                description="Une ligne par localité. Le formulaire regroupe automatiquement les localités par région."
                itemLabel={(it) => `${it.region ?? ""} — ${it.commune ?? ""}`}
                fields={[
            ...
            <div className="mt-3 space-y-5">
              <ObjectBlockAdmin
                ...
                defaultValue={{
                  ...
                  titre: {
                    fr: "Cadre réglementaire du secteur",
                    en: "Sector regulatory framework",
                    ar: "الإطار التنظيمي للقطاع",
                  },
                  description: {
                    fr: "Consultez et téléchargez l'ensemble des textes en vigueur applicables aux postes et aux télécommunications.",
                    en: "Browse and download all texts currently in force applicable to posts and telecommunications.",
                    ar: "اطّلع على جميع النصوص السارية المطبقة على البريد والاتصالات وقم بتحميلها.",
                  },
                }}
              />
            </div>
            ...
            <div className="mt-3 space-y-5">
              <ObjectBlockAdmin
                ...
                defaultValue={{
                  ...
                  titre: {
                    fr: "Équipements et terminaux homologués",
                    en: "Approved equipment and terminals",
                    ar: "المعدات والأجهزة المعتمدة",
                  },
                  description: {
                    fr: "Avant tout achat ou importation, vérifiez le statut d'homologation d'un équipement radioélectrique auprès de l'ARPT.",
                    en: "Before any purchase or import, check the approval status of a radio equipment with ARPT.",
                    ar: "قبل أي شراء أو استيراد، تحقق من حالة اعتماد الجهاز اللاسلكي لدى الهيئة.",
                  },
                }}
              />
            </div>
            ...
            <div className="mt-3 space-y-5">
              <ObjectBlockAdmin
                ...
                defaultValue={{
                  ...
                  titre: {
                    fr: "Appels d'offres de l'Autorité",
                    en: "Authority tenders",
                    ar: "مناقصات الهيئة",
                  },
                  description: {
                    fr: "Les avis publiés ci-dessous précisent l'objet du marché, le budget prévisionnel et la date limite de dépôt des plis.",
                    en: "The notices published below specify the subject of the contract, the estimated budget and the submission deadline.",
                    ar: "توضح الإعلانات المنشورة أدناه موضوع الصفقة والميزانية التقديرية والموعد النهائي لإيداع الملفات.",
                  },
                }}
              />
            </div>
            ...
            <div className="mt-3 space-y-5">
              <ObjectBlockAdmin
                ...
                defaultValue={{
                  ...
                  titre: {
                    fr: "Offres d’emploi de l’ARPT",
                    en: "Careers at ARPT",
                    ar: "الوظائف في الهيئة",
                  },
                  description: {
                    fr: "L'Autorité recrute des profils techniques, juridiques et économiques engagés au service du secteur numérique guinéen.",
                    en: "The Authority recruits technical, legal and economic profiles committed to serving Guinea's digital sector.",
                    ar: "تعمل الهيئة على توظيف كفاءات تقنية وقانونية واقتصادية ملتزمة بخدمة القطاع الرقمي الغيني.",
                  },
                }}
              />
            </div>
            ...
            <div className="mt-3 space-y-5">
              <ObjectBlockAdmin
                ...
                defaultValue={{
                  ...
                  titre: {
                    fr: "Actualités et communiqués",
                    en: "News and press releases",
                    ar: "الأخبار والبلاغات",
                  },
                  description: {
            ...
            <div className="mt-3 space-y-5">
              <ObjectBlockAdmin
                ...
                defaultValue={{
                  ...
                  titre: {
                    fr: "Services aux opérateurs, entreprises et particuliers",
                    en: "Services for operators, businesses and individuals",
                    ar: "خدمات للمشغلين والشركات والأفراد",
                  },
                  description: {
                    fr: "Pour chaque service, retrouvez les pièces exigées, le délai d'instruction et le coût applicable. Les demandes se déposent en ligne depuis le portail usager.",
                    en: "For each service, find the required documents, processing time and applicable cost. Requests are submitted online via the user portal.",
                    ar: "لكل خدمة، تجد الوثائق المطلوبة ومدة المعالجة والتكلفة المطبقة. تُقدَّم الطلبات عبر الإنترنت من خلال بوابة المستخدم.",
                  },
                }}
              />
            </div>
            ...
            <div className="mt-3 space-y-5">
              <ObjectBlockAdmin
                ...
                defaultValue={{
                  ...
                  description: {
                    fr: "Une question sur une démarche, un texte réglementaire ou un dossier en cours ? Nos services vous répondent sous cinq jours ouvrés.",
                    en: "A question about a procedure, a regulatory text or an ongoing case? Our teams respond within five business days.",
                    ar: "هل لديك سؤال حول إجراء أو نص تنظيمي أو ملف قيد المعالجة؟ تجيبكم مصالحنا خلال خمسة أيام عمل.",
                  },
                }}
              />
            </div>
            ...
            <div className="mt-3 space-y-5">
              <ObjectBlockAdmin
                ...
                defaultValue={{
                  ...
                  description: {
                    fr: "Indicateurs mensuels et trimestriels consolidés par l'Autorité à partir des déclarations des opérateurs.",
                    en: "Monthly and quarterly indicators consolidated by the Authority from operator filings.",
                    ar: "مؤشرات شهرية وفصلية جمعتها الهيئة من تصريحات المشغلين.",
                  },
                }}
              />
            </div>
        ...
        {pageSelectionnee === "consultations" && (
          <section>
            <p className="text-xs font-semibold tracking-wide text-muted-foreground uppercase">
              {t(`pages.consultations`)}
            </p>
            <div className="mt-3 space-y-5">
              <ObjectBlockAdmin
                ...
                defaultValue={{
                  ...
                  titre: {
                    fr: "Consultations publiques",
                    en: "Public consultations",
                    ar: "الاستشارات العمومية",
                  },
                  description: {
                    fr: "Avant l'adoption d'un texte structurant, l'Autorité recueille les observations des opérateurs, des associations de consommateurs et du public.",
                    en: "Before adopting a major text, the Authority gathers input from operators, consumer associations and the public.",
                    ar: "قبل اعتماد نص هيكلي، تجمع الهيئة ملاحظات المشغلين وجمعيات المستهلكين والجمهور.",
                  },
                }}
              />
            </div>
...
function ActualitesAdmin() {
  ...
  const {
    ...
    error,
    refetch,
  } = useApiList<NewsAdmin>("/news?lang=fr&pageSize=100");
  const [showForm, setShowForm] = useState(false);
    ...
    .filter((item) => {
      const correspondRecherche =
        `${item.title} ${item.content} ${item.category ?? ""}`
          ...
          .includes(recherche.trim().toLocaleLowerCase("fr"));
      const correspondCategorie =
        categorie === "all" || item.category === categorie;
      const correspondStatut =
        statut === "all" ||
        (statut === "published" ? item.isPublished : !item.isPublished);
  ...
  function exporter(format: ExportFormat = "csv") {
    const headers = (t.raw("csv.headers") as string[]).filter(
      (_, index) => index !== 3,
    );
    exporterTableau(
      format,
      ...
      <div className="rounded-2xl border border-border bg-card shadow-card">
        <div className="flex flex-col gap-4 border-b border-border p-5 sm:p-6 lg:flex-row lg:items-start lg:justify-between">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <Button size="sm" onClick={ouvrirCreation}>
              {showForm && !editing ? tCommon("cancel") : t("newArticle")}
            </Button>
          </div>
        </div>
        ...
        {showForm && (
          <form
            key={editing?.id ?? "new"}
            onSubmit={soumettre}
            className="mt-4 grid gap-4 rounded-xl border border-border bg-card p-5"
          >
            <p className="text-sm font-semibold">
              {editing
                ? t("editingTitle", { name: editing.title })
                : t("newArticleTitle")}
            </p>
            <div className="grid gap-2">
            ...
            <div className="grid gap-2">
              <Label htmlFor="news-image">
                {t("fields.image", {
                  hint: editing ? t("fields.imageHintKeep") : "*",
                })}
              </Label>
              <Input
                id="news-image"
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[55rem] text-left text-sm">
            <thead className="border-b border-border bg-card text-[10px] font-semibold tracking-wide text-muted-foreground uppercase">
              <tr>
            ...
            <tbody className="divide-y divide-border">
              {actualitesFiltrees.map((n) => (
                <tr key={n.id} className="transition-colors hover:bg-accent/25">
                  ...
                  <td className="px-4 py-3.5">
                    {n.category ? (
                      <span className="inline-flex rounded-full bg-accent px-2 py-1 text-[10px] font-semibold text-accent-foreground">
                        {n.category}
                      </span>
                    ) : (
                  ...
                  <td className="px-4 py-3.5 text-xs text-muted-foreground">
                    <p>{formaterDate(n.createdAt)}</p>
                  </td>
                  <td className="px-4 py-3.5">
                    {n.isPublished ? (
                      <Puce label={t("statusPublished")} tone="success" />
                    ) : (
                      <Puce label={t("statusDraft")} tone="warning" />
                    )}
                  </td>
                  <td className="px-5 py-3.5">
                    <div className="flex justify-end gap-1">
                      <button
                        ...
                        title={tCommon("edit")}
                        className="grid size-8 place-items-center rounded-md text-muted-foreground hover:bg-accent hover:text-primary"
                      >
...
function CommuniquesAdmin() {
  ...
  const {
    ...
    error,
    refetch,
  } = useApiList<CommuniqueAdmin>("/communiques?lang=fr&pageSize=100");
  const [showForm, setShowForm] = useState(false);
  ...
  const resultat = communiques.filter((c) => {
    const correspondRecherche = c.title
      .toLocaleLowerCase("fr")
      .includes(recherche.trim().toLocaleLowerCase("fr"));
  ...
  const nouveauxCetteSemaine = communiques.filter((c) =>
    depuisMoinsDuneSemaine(c.createdAt),
  ).length;
    ...
    <section className="mt-8 space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
        ...
        <StatTrendCard
          icon={Paperclip}
          tone="success"
          value={avecPieceJointe}
          label={t("stats.withAttachment")}
        />
        <StatTrendCard
          icon={FileText}
          tone="warning"
          value={communiques.length - avecPieceJointe}
          label={t("stats.withoutAttachment")}
        />
      </div>
...
function ServicesAdmin() {
  ...
  const {
    ...
    error,
    refetch,
  } = useApiList<ServiceAdmin>("/services?lang=fr&pageSize=100");
  const [showForm, setShowForm] = useState(false);
  ...
  const [formError, setFormError] = useState("");
  const [recherche, setRecherche] = useState("");
  const [filtreEtat, setFiltreEtat] = useState<"all" | "actif" | "inactif">(
    "all",
  );
  const [page, setPage] = useState(1);
  ...
  const resultat = services.filter((s) => {
    const correspondRecherche = `${s.name} ${s.description ?? ""}`
      .toLocaleLowerCase("fr")
      .includes(recherche.trim().toLocaleLowerCase("fr"));
    const correspondEtat =
      filtreEtat === "all" ||
      (filtreEtat === "actif" ? s.isActive : !s.isActive);
  ...
  const nouveauxCetteSemaine = services.filter((s) =>
    depuisMoinsDuneSemaine(s.createdAt),
  ).length;
  ...
  async function soumettre(event: React.FormEvent<HTMLFormElement>) {
    ...
    const body = new FormData();
    body.append("nameFr", String(form.get("nameFr")));
    if (form.get("descriptionFr"))
      body.append("descriptionFr", String(form.get("descriptionFr")));
    if (form.get("delaiFr"))
      body.append("delaiFr", String(form.get("delaiFr")));
        ...
        >
          ...
          <div className="grid gap-2">
            ...
            <Input
              id="svc-delai"
              name="delaiFr"
              placeholder={t("fields.delayPlaceholder")}
              defaultValue={editing?.delai ?? ""}
            />
          </div>
          <div className="grid gap-2">
            ...
            <Input
              ...
              name="cost"
              type="number"
              placeholder={t("fields.costOptional")}
              defaultValue={editing?.cost ?? ""}
            />
          </div>
          <div className="grid gap-2 sm:col-span-2">
            <Label htmlFor="svc-documents">
              {t("fields.requiredDocuments")}
            </Label>
            <Input
              id="svc-documents"
          ...
          <div className="grid gap-2">
            <Label htmlFor="svc-image">
              {t("fields.image", {
                hint: editing
                  ? t("fields.imageHintKeep")
                  : t("fields.imageHintOptional"),
              })}
            </Label>
            ...
            {editing?.imageUrl && (
              <img
                src={editing.imageUrl}
                alt=""
                className="mt-1 h-20 w-auto rounded-lg border border-border object-cover"
              />
            )}
          </div>
          <div className="grid gap-2">
            <Label htmlFor="svc-file">
              {t("fields.file", {
                hint: editing
                  ? t("fields.fileHintKeep")
                  : t("fields.fileHintOptional"),
      ...
      <div className="overflow-hidden rounded-2xl border border-border bg-card shadow-card">
        <div className="grid gap-3 border-b border-border bg-surface/55 p-4 sm:grid-cols-2 lg:grid-cols-[minmax(12rem,1fr)_10rem_auto]">
          ...
          <Select
            value={filtreEtat}
            onValueChange={(value) => {
              setFiltreEtat(value as typeof filtreEtat);
              setPage(1);
            }}
          >
            <SelectTrigger className="h-9 bg-card text-xs">
        ...
        <TableauAdmin
          ...
          lignes={resultatPage.map((s) => [
            ...
            s.delai ?? "—",
            s.isActive ? (
              <Puce key={s.id} label={tCommon("active")} tone="success" />
            ) : (
              <Puce key={s.id} label={tCommon("inactive")} tone="warning" />
            ),
            <RowActions
              key={s.id}
...
function MessageRow({
  ...
  return (
    ...
    >
      {!message.isRead && !message.isArchived && (
        <span
          className="absolute left-0 top-6 h-9 w-1 rounded-r-full bg-primary"
      ...
      <div className="flex gap-3.5">
        <div
          className={cn(
            ...
            !message.isRead && !message.isArchived
              ? "bg-institution text-primary-foreground"
        ...
        <div className="min-w-0 flex-1">
          <div className="flex flex-wrap items-start justify-between gap-x-4 gap-y-2">
            ...
            <div className="flex shrink-0 items-center gap-2">
              ...
              {message.isArchived ? (
                <Puce label={t("archived")} tone="info" />
              ) : !message.isRead ? (
                <Puce label={t("new")} tone="warning" />
              ) : null}
            </div>
          </div>
          ...
          <div className="mt-4 flex flex-wrap items-center gap-2">
            ...
            >
              ...
              {showReply ? t("closeReply") : t("reply")}
            </Button>
            <button
              type="button"
            ...
            >
              ...
              <span className="sr-only">
                {t("markAs", {
                  state: message.isRead ? t("stateUnread") : t("stateRead"),
                })}
              </span>
            </button>
            <button
      ...
      {showReply && (
        <form
          onSubmit={envoyer}
          className="ml-0 mt-5 grid gap-3 rounded-xl border border-primary/15 bg-surface p-4 sm:ml-[3.4rem]"
        >
          <Label
            htmlFor={`message-reply-${message.id}`}
            className="flex items-center gap-2 text-xs text-muted-foreground"
          >
            <Reply className="size-3.5 text-primary" aria-hidden />{" "}
            {t("replyTo", { name: message.name })}
          </Label>
          <Textarea
          ...
          >
            ...
            {sending ? t("sending") : t("sendByEmail")}
          </Button>
        </form>
      )}
    </article>
...
function MessagesAdmin() {
  ...
  const {
    ...
    error,
    refetch,
  } = useApiList<ContactMessageAdmin>("/contact-messages?pageSize=100");
  const [recherche, setRecherche] = useState("");
  const [filtre, setFiltre] = useState<"all" | "unread" | "archived">("all");

  const messagesFiltres = messages.filter((message) => {
    const termes =
      `${message.name} ${message.email} ${message.subject} ${message.message}`.toLocaleLowerCase(
        "fr",
      );
    const correspondRecherche = termes.includes(
      recherche.trim().toLocaleLowerCase("fr"),
  ...
  const nonLus = messages.filter(
    (message) => !message.isRead && !message.isArchived,
  ).length;
  ...
  return (
    <section className="mt-8 overflow-hidden rounded-2xl border border-border bg-card shadow-soft">
      <div className="flex flex-col gap-4 border-b border-border bg-surface/70 px-4 py-4 sm:px-6 lg:flex-row lg:items-center lg:justify-between">
        <div>
          <div className="flex items-center gap-2">
            <span className="grid size-8 place-items-center rounded-lg bg-teal-600 text-white shadow transition-colors hover:bg-teal-700">
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            {t("receivedCount", { count: messages.length })}
          </p>
        </div>
      ...
      {messagesFiltres.length === 0 && (
        <div className="px-5 py-14 text-center">
          ...
          <p className="mt-3 text-sm font-medium">
            {messages.length === 0 ? t("emptyInbox") : t("noSearchResults")}
          </p>
          {messages.length > 0 && (
            <button
              type="button"
...
function ReglementationAdmin() {
  ...
  const {
    ...
    error,
    refetch,
  } = useApiList<ReglementationAdmin>("/regulations?lang=fr&pageSize=100");
  const [showForm, setShowForm] = useState(false);
  ...
  const categoriesNoms = Array.from(
    new Set(textes.map((texte) => texte.category)),
  );
  ...
  const resultat = textes.filter((texte) => {
    const correspondRecherche = texte.name
      .toLocaleLowerCase("fr")
      .includes(recherche.trim().toLocaleLowerCase("fr"));
    const correspondCategorie =
      filtreCategorie === "all" || texte.category === filtreCategorie;
  ...
  const nouveauxCetteSemaine = textes.filter((texte) =>
    depuisMoinsDuneSemaine(texte.dateUpload),
  ).length;
        ...
        >
          ...
          <div className="grid gap-2 sm:col-span-2">
            ...
            <div className="rounded-lg border border-dashed border-border bg-surface p-3">
              <p className="text-[10px] font-semibold tracking-wide text-muted-foreground uppercase">
                {t("fields.sitePreview")}
              </p>
              <p className="mt-1.5 text-sm leading-relaxed text-muted-foreground">
                {description || (
                  <span className="italic text-muted-foreground/60">
                    {t("fields.noDescriptionYet")}
                  </span>
                )}
              </p>
            </div>
          ...
          <div className="grid gap-2">
            ...
            <Input
              ...
              type="date"
              required
              defaultValue={editing?.dateUpload?.slice(0, 10)}
            />
          </div>
          <label className="flex items-center gap-2 text-sm sm:col-span-2">
            <input
          ...
          {editing?.fileUrl && (
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="text-sm text-primary underline sm:col-span-2"
            >
              {t("viewCurrentFile")}
            </a>
          )}
          <div className="grid gap-2 sm:col-span-2">
            <Label htmlFor="reg-file">
              {t("fields.file", {
                hint: editing ? t("fields.fileHintKeep") : "*",
              })}
            </Label>
            <Input
              id="reg-file"
      ...
      <div className="overflow-hidden rounded-2xl border border-border bg-card shadow-card">
        ...
        <TableauAdmin
          ...
          lignes={resultatPage.map((texte) => [
            ...
            <RowActions
              key={texte.id}
              onEdit={() => ouvrirEdition(texte)}
              onDelete={() => supprimer(texte)}
            />,
          ])}
        />
        {resultatPage.length === 0 && (
...
function ConsultationsAdmin() {
  ...
  const resultat = consultations.filter((c) => {
    const correspondRecherche = c.title
      ...
      .includes(recherche.trim().toLocaleLowerCase("fr"));
    const correspondStatut =
      filtreStatut === "all" || c.status === filtreStatut;
  ...
  const nouvellesCetteSemaine = consultations.filter((c) =>
    depuisMoinsDuneSemaine(c.createdAt),
  ).length;
  ...
  function exporter(format: ExportFormat = "csv") {
    exporterTableau(
      ...
      resultat.map((c) => [
        c.title,
        tStatus(c.status),
        formaterDate(c.startDate),
        formaterDate(c.endDate),
      ]),
    );
  }
  ...
  async function soumettre(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      if (editing) {
        await apiFetch(`/public-consultations/${editing.id}`, {
          method: "PATCH",
          body,
        });
        toast.success(t("updated"));
...
function UserRoleSelect({
  ...
  async function changer(value: string) {
    ...
    try {
      await apiFetch(`/users/${user.id}`, {
        method: "PATCH",
        body: JSON.stringify({
          roleId: value === "none" ? null : Number(value),
        }),
      });
  ...
  return (
    <div className="flex items-center gap-2">
      <Select
        value={user.role ? String(user.role.id) : "none"}
        ...
        <dl className="px-5 py-2">
          ...
          {user.accountType === "ENTREPRISE" && (
            <>
              <DetailRow
                label={t("companyName")}
                value={user.companyName ?? na}
              />
              <DetailRow
                label={t("enterpriseStatus")}
                value={
                  user.enterpriseApprovalStatus
                    ? tEnterpriseStatus(user.enterpriseApprovalStatus)
                    : na
                }
              />
              ...
              <DetailRow
                label={t("document")}
                value={
                  user.hasCompanyDocument ? (
                    <EntrepriseDocumentLink userId={user.id} />
                  ) : (
                    na
                  )
                }
          ...
          <DetailRow
            ...
            value={
              user.isSuperuser
                ? tRole("superAdmin")
                : (user.role?.name ?? tRole("noRole"))
            }
          />
          <DetailRow
            label={t("state")}
            value={user.isActive ? tCommon("active") : tCommon("inactive")}
          />
          <DetailRow
            label={t("joinedOn")}
            value={formaterDate(user.dateJoined)}
          />
        </dl>
  ...
  return (
    <div>
      <div className="flex items-center justify-between gap-4">
        ...
        <Button
          size="sm"
          onClick={() => (showForm ? fermerForm() : ouvrirCreation())}
        >
...
function EnterpriseApprovalActions({
  user,
  onUpdated,
}: {
  user: UserAdmin;
  ...
  async function decide(decision: "APPROUVE" | "REJETE") {
    const reason =
      decision === "REJETE" ? window.prompt(t("rejectPrompt")) : undefined;
...
function CreerUtilisateurAdmin({
  roles,
  onCreated,
}: {
  roles: RoleOption[];
  ...
  return (
    <div>
      ...
      {showForm && (
        <form
          onSubmit={creer}
          className="mt-5 grid gap-4 rounded-xl border border-primary/15 bg-surface p-5 shadow-soft sm:grid-cols-2"
        >
          <div className="grid gap-2">
...
function UtilisateursAdmin() {
  ...
  const {
    ...
    error,
    refetch,
  } = useApiList<UserAdmin>("/users?pageSize=100");
  const { data: roles } = useApiOne<RoleOption[]>("/roles");
  const [recherche, setRecherche] = useState("");
  const [filtreType, setFiltreType] = useState<
    "all" | UserAdmin["accountType"]
  >("all");
  const [filtreEtat, setFiltreEtat] = useState<"all" | "active" | "inactive">(
    "all",
  );

  const comptesFiltres = users.filter((user) => {
    const texte =
      `${user.fullname} ${user.email} ${user.companyName ?? ""} ${user.role?.name ?? ""}`.toLocaleLowerCase(
        "fr",
      );
    return (
      ...
      (filtreEtat === "all" ||
        (filtreEtat === "active" ? user.isActive : !user.isActive))
    );
  });
  const comptesEntreprise = users.filter(
    (user) => user.accountType === "ENTREPRISE",
  ).length;
  const comptesActifs = users.filter((user) => user.isActive).length;
  const enAttente = users.filter(
    (user) => user.enterpriseApprovalStatus === "EN_ATTENTE",
  ).length;
    ...
    <div className="mt-8 grid gap-6">
      <section className="overflow-hidden rounded-2xl border border-border bg-card shadow-card">
        <div className="bg-institution px-5 py-6 text-primary-foreground sm:px-6">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div className="flex items-start gap-3">
              <span className="grid size-11 place-items-center rounded-xl bg-teal-600 text-white shadow transition-colors hover:bg-teal-700">
                <Users className="size-5" aria-hidden />
              </span>
              <div>
                <p className="font-heading text-xs font-semibold tracking-[0.16em] uppercase opacity-80">
                  {t("eyebrow")}
                </p>
            ...
            {enAttente > 0 && (
              <span className="rounded-full bg-primary-foreground/15 px-3 py-1.5 text-xs font-semibold">
                {t("pendingEnterprises", { count: enAttente })}
              </span>
            )}
          </div>
          <div className="mt-6 grid grid-cols-2 gap-px overflow-hidden rounded-xl bg-primary-foreground/15 sm:grid-cols-4">
            {[
              ...
              <div
                key={String(libelle)}
                className="bg-primary-deep/25 px-4 py-3"
              >
            ...
            <div className="flex flex-wrap gap-2">
              <div className="flex rounded-lg bg-muted p-1">
                {(
                  [
                    ["all", t("typeAll")],
                    ["PARTICULIER", t("typeIndividual")],
                    ["ENTREPRISE", t("typeEnterprise")],
                  ] as const
                ).map(([valeur, libelle]) => (
                  <button
                    key={valeur}
            ...
            <TableauAdmin
              ...
              lignes={comptesFiltres.map((u) => [
                ...
                <div key={`profile-${u.id}`} className="min-w-32">
                  <Puce
                    label={
                      u.accountType === "ENTREPRISE"
                        ? tDetail("enterprise")
                        : tDetail("individual")
                    }
                    tone={u.accountType === "ENTREPRISE" ? "info" : "success"}
                  />
                  {u.accountType === "ENTREPRISE" && (
                  ...
                  {u.accountType === "ENTREPRISE" && (
                    <EnterpriseApprovalActions user={u} onUpdated={refetch} />
                  )}
                </div>,
                <UserRoleSelect
                  key={`role-${u.id}`}
                ...
                >
                  <UserActiveToggle user={u} onUpdated={refetch} />
                  {!u.emailVerified && (
...
function AdminLogin() {
  ...
  return (
    <div className="grid h-full overflow-y-auto bg-surface-fade p-4 sm:p-6 lg:p-10">
      <div className="m-auto grid w-full max-w-5xl overflow-hidden rounded-2xl border border-border bg-card shadow-lifted lg:grid-cols-[minmax(0,1.08fr)_minmax(24rem,0.92fr)]">
        <div className="relative min-h-52 overflow-hidden bg-teal-950 sm:min-h-64 lg:min-h-144">
          <Image
            src="/images/adminlogin.png"
            alt="Technicien des télécommunications dans un centre de supervision"
            fill
            priority
            className="object-cover object-center"
            sizes="(max-width: 1023px) 100vw, 55vw"
          />
          <div className="absolute inset-x-0 bottom-0 bg-linear-to-t from-teal-950/90 via-teal-950/25 to-transparent px-6 pb-6 pt-20 text-white sm:px-8 sm:pb-8">
            ...
            <p className="mt-2 max-w-sm text-sm leading-6 text-white/85">
              Espace sécurisé de supervision et de pilotage des services
              régulés.
            </p>
          </div>
        </div>
