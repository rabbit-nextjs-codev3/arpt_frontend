...
const missions = [
  ...
  },
  {
    ...
    texte: "Planifier et attribuer le spectre radioélectrique ainsi que les ressources en numérotation.",
  },
  {
    icon: Users,
    titre: "Protéger les consommateurs",
    texte: "Traiter les réclamations, contrôler la qualité de service et informer les usagers de leurs droits.",
  },
  {
    icon: ShieldCheck,
    titre: "Sécuriser le secteur",
    texte: "Homologuer les équipements, contrôler les opérateurs et veiller au respect du cadre légal.",
  },
];
    ...
    <>
      ...
      <section className="section-y">
        <div className="container-content grid gap-12 lg:grid-cols-[1.1fr_0.9fr] lg:items-center">
          <div>
            <SectionTitle surtitre="Direction générale" titre="Une équipe au service du secteur" description="Découvrez les responsables qui portent la mission de l'ARPT et accompagnent la transformation numérique de la Guinée." />
            <div className="mt-8"><MemberCarousel /></div>
          </div>
          <aside className="rounded-2xl border border-border bg-surface p-6 lg:p-8">
            ...
            <p className="mt-3 text-sm leading-6 text-muted-foreground">Notre équipe vous oriente vers le bon service pour vos démarches, vos réclamations et vos questions réglementaires.</p>
            <div className="mt-7 grid gap-4 border-t border-border pt-5 text-sm">
              <a href="tel:+224669221000" className="flex items-center gap-3 font-medium hover:text-primary"><Phone className="size-4 text-primary" aria-hidden /> +224 669 221 000</a>
              <a href="mailto:contact@arpt.gov.gn" className="flex items-center gap-3 font-medium hover:text-primary"><Mail className="size-4 text-primary" aria-hidden /> contact@arpt.gov.gn</a>
            </div>
          </aside>
        </div>
      </section>
