...
function isPdfTable(value: unknown): value is PdfTable {
  ...
  if (!Array.isArray(columns) || columns.length < 1 || columns.length > 12)
    return false;
  if (!columns.every((cell) => typeof cell === "string" && cell.length <= 100))
    return false;
...
export async function POST(request: Request) {
  ...
  try {
    ...
    return Response.json(
      { message: "Vérification de la session indisponible." },
      { status: 503 },
    );
  }
  ...
  if (Number(request.headers.get("content-length") ?? 0) > 2_000_000)
    return Response.json(
      { message: "Rapport trop volumineux." },
      { status: 413 },
    );
  let raw: string;
  try {
  ...
  if (activeRenders >= 2)
    return Response.json(
      { message: "Génération en cours, réessayez." },
      { status: 503 },
    );
