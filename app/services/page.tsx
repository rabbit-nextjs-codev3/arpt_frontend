...
export default function Services() {
  ...
  const {
    ...
    loading,
    error,
  } = useApiList<Service>("/services?lang=fr&pageSize=50");
    ...
    <>
      ...
      <section className="section-y bg-surface-fade">
        <div className="container-content">
          {loading && (
            <Loader className="flex justify-center py-8" label={t("loading")} />
          )}
          {error && !loading && (
          ...
          <div className="mt-7 grid gap-5 lg:grid-cols-2">
            {services.map((s) => (
              <article
                key={s.uid}
                className="group flex flex-col rounded-2xl border border-border bg-card p-5 shadow-soft transition-all hover:-translate-y-0.5 hover:border-primary/30 hover:shadow-card sm:p-6"
              >
                ...
                <h2 className="mt-5 font-heading text-xl font-semibold transition-colors group-hover:text-primary">
                  {s.name}
                </h2>
                ...
                <dl className="mt-6 grid gap-3 rounded-xl bg-surface p-3 sm:grid-cols-2">
                  <div className="flex items-start gap-2 py-1 text-sm">
                    ...
                    />
                    <div>
                      <dt className="text-[11px] font-semibold text-muted-foreground uppercase">
                        {t("delay")}
                      </dt>
                      <dd className="mt-0.5 font-medium">{s.delai}</dd>
                    </div>
                  </div>
                  <div className="flex items-start gap-2 py-1 text-sm">
                    <Coins
                ...
                <Button asChild className="mt-7 self-start">
                  <Link href={`/contact?service=${s.uid}`}>
                    {t("submitRequest")}{" "}
