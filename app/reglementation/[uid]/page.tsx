      ...
      <div className="container-content grid gap-12 lg:grid-cols-[minmax(0,1fr)_20rem] lg:items-start">
          ...
          <div className="mt-8 overflow-hidden rounded-2xl border border-border bg-card shadow-soft">
            <div className="px-6 py-8 sm:px-10 sm:py-10 lg:px-14 lg:py-12">
              <div className="flex flex-wrap items-center gap-3 text-xs text-muted-foreground">
                ...
                <span className="inline-flex items-center gap-1.5"><CalendarDays className="size-3.5" aria-hidden /> {formaterDate(texte.dateUpload)}</span>
                <span className="inline-flex items-center gap-1.5"><Eye className="size-3.5" aria-hidden /> {t("views", { count: texte.views.toLocaleString(locale) })}</span>
                <span>{texte.format}</span>
              </div>
              <h1 className="mt-5 max-w-4xl text-2xl leading-tight font-bold tracking-tight md:text-4xl">{texte.name}</h1>
              {texte.description && (
        ...
        <aside className="lg:sticky lg:top-8">
          <div className="border-t-2 border-foreground pt-4">
            <p className="font-heading text-xs font-semibold tracking-[0.16em] text-primary uppercase">{t("sameCategory")}</p>
            <h2 className="mt-2 font-heading text-xl font-semibold">{t("otherTexts")}</h2>
          </div>
          <div className="mt-6 divide-y divide-border">
            {related.map((item) => (
              <Link key={item.uid} href={`/reglementation/${item.uid}`} className="group block py-5 first:pt-0">
                <div className="flex items-start gap-3">
                  <FileText className="mt-0.5 size-4 shrink-0 text-muted-foreground" aria-hidden />
                  <div>
                    <h3 className="font-heading text-sm leading-snug font-semibold transition-colors group-hover:text-primary">{item.name}</h3>
                    <span className="mt-1.5 block text-xs text-muted-foreground">{formaterDate(item.dateUpload)}</span>
                  </div>
                </div>
              </Link>
            ))}
            {related.length === 0 && (
              <p className="py-5 text-sm text-muted-foreground">{t("noOtherText")}</p>
            )}
          </div>
        </aside>
      </div>
