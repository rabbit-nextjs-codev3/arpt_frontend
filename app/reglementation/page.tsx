...
export default function Reglementation() {
  ...
  useEffect(() => {
    ...
    api
      .get<PaginatedResult<Reglementation>>(
        `/regulations?lang=${locale}&pageSize=100`,
      )
      .then((res) => {
      ...
      .catch((err: unknown) => {
        if (!annule)
          setErreur(err instanceof ApiError ? err.message : t("loadError"));
      })
      .finally(() => {
    ...
    <>
      ...
      <section className="section-y bg-surface-fade">
        <div className="container-content">
          {/* Sticky Header Card containing Search and Filter Pills */}
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="mt-1 text-sm text-muted-foreground">
                    {resultats.length} texte{resultats.length > 1 ? "s" : ""}{" "}
                    disponible{resultats.length > 1 ? "s" : ""}
                  </p>
                </div>
              </div>
            ...
            {!chargement && !erreur && (
              <ul className="divide-y divide-border">
                {resultats.map((r) => (
                  <li
                    key={r.uid}
                    className="grid gap-4 p-5 transition-colors hover:translate-x-1 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center"
                  >
                    <div className="min-w-0">
                      ...
                      <p className="mt-1.5 flex flex-wrap items-center gap-3 text-xs text-muted-foreground">
                        <span className="rounded bg-teal-100 px-2 py-0.5 font-semibold text-accent-foreground">
                          {r.category}
                        </span>
                        ...
                        <span className="inline-flex items-center gap-1">
                          ...
                          {t("views", {
                            count: r.views.toLocaleString(locale),
                          })}
                        </span>
                        <span>{r.format}</span>
                      </p>
