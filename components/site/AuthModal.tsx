        ...
        {open && (
          <div className="relative p-6 sm:p-8">
            <button
              ...
              className="absolute right-3 top-3 grid size-10 place-items-center rounded-full text-muted-foreground transition-colors hover:bg-muted hover:text-foreground focus-visible:outline-2 focus-visible:outline-ring"
            >
              <X className="size-5" aria-hidden />
            </button>
            <div className="mb-5 text-center">
              <span className="mx-auto mb-3 grid size-10 place-items-center rounded-xl bg-teal-600 text-white shadow transition-colors hover:bg-teal-700">
                {view === "auth" ? (
                  <ShieldCheck
                    className="size-5"
                    strokeWidth={1.7}
              ...
              >
                {view === "auth"
                  ? "Un seul compte pour vos démarches, vos réclamations et leur suivi."
            ...
            {view === "auth" ? (
              <Tabs defaultValue="signin" className="w-full">
                <TabsList
                  ...
                  className="mb-5 grid h-12 w-full grid-cols-2 rounded-xl bg-muted p-1"
                >
                  <TabsTrigger value="signin" className="h-10 rounded-lg">
                    Connexion
                  </TabsTrigger>
                  <TabsTrigger value="signup" className="h-10 rounded-lg">
                    Inscription
                  </TabsTrigger>
                </TabsList>
                ...
                <TabsContent value="signup">
                  <AuthForm mode="signup" onSuccess={() => setOpen(false)} />
                </TabsContent>
              </Tabs>
            ) : (
            ...
            <p className="mt-4 flex items-center justify-center gap-2 text-xs text-muted-foreground">
              ...
              Votre espace personnel ARPT Guinée
            </p>
          </div>
        )}
      </dialog>
    ...
    <div className="grid gap-2">
      ...
      <div className="relative">
        <Input
          ...
          placeholder={signup ? "Au moins 8 caractères" : "Votre mot de passe"}
          aria-describedby={signup && !confirm ? `${id}-hint` : undefined}
          className="h-12 rounded-lg bg-card pr-12 shadow-none"
        />
        <button
          type="button"
          aria-label={`${visible ? "Masquer" : "Afficher"} ${confirm ? "la confirmation" : "le mot de passe"}`}
          aria-pressed={visible}
          onClick={() => setVisible(!visible)}
          className="absolute right-1 top-1 grid size-10 place-items-center rounded-md text-muted-foreground hover:bg-muted focus-visible:outline-2 focus-visible:outline-ring"
        >
          {visible ? (
...
function AuthForm({
  ...
  const [accountType, setAccountType] = useState<"particulier" | "entreprise">(
    "particulier",
  );
  const [error, setError] = useState("");
  const [submitting, setSubmitting] = useState(false);
  const [enterprisePending, setEnterprisePending] = useState<string | null>(
    null,
  );
  ...
  if (enterprisePending) {
    return (
      <div className="grid gap-4 text-center">
        ...
        <Button
          type="button"
          onClick={() => setEnterprisePending(null)}
          variant="outline"
  ...
  return (
    ...
    >
      ...
      {signup && (
        <div className="grid gap-2">
          ...
          <Input
            ...
            className="h-12 rounded-lg bg-card shadow-none"
          />
        </div>
      )}
      ...
      {signup && accountType === "entreprise" && (
        <div className="grid gap-2 sm:col-span-2">
          <Label htmlFor={`${id}-company-document`}>
            Document justificatif (RCCM ou équivalent)
          </Label>
          <Input
            id={`${id}-company-document`}
          ...
          <p className="text-xs text-muted-foreground">
            PDF, JPG ou PNG. Votre compte sera activé après validation par un
            administrateur.
          </p>
        </div>
      )}
      ...
      <Button
        type="submit"
        disabled={submitting}
        className="mt-1 h-12 w-full rounded-lg text-sm font-semibold sm:col-span-2"
      >
        {submitting
          ? "Veuillez patienter…"
          : signup
      ...
      {signup ? (
        <p className="text-center text-xs leading-5 text-muted-foreground sm:col-span-2">
          Créez votre compte pour retrouver vos démarches au même endroit.
        </p>
      ) : (
        <p className="text-center text-xs leading-5 text-muted-foreground sm:col-span-2">
  ...
  return (
    <div className="mx-auto max-w-md">
      {sent ? (
        <p
          role="status"
          className="rounded-xl border border-border bg-muted p-5 text-center text-sm leading-6"
        >
          Si un compte est associé à cette adresse, vous recevrez un e-mail avec
        ...
        <form onSubmit={submit} className="grid gap-4">
          ...
          >
            {loading ? "Veuillez patienter…" : "Recevoir les instructions"}
            <ArrowRight className="size-4" aria-hidden />
          </Button>
        </form>
      )}
      <button
        type="button"
        onClick={onBack}
        className="mx-auto mt-5 flex items-center gap-2 rounded-md px-2 py-1 text-sm font-medium text-primary hover:underline"
      >
